...
- fix: `classifyError` now checks the CreditsError/no-payment-method message shape BEFORE the generic `status===401/403` check - a provider that returns HTTP 401 (not 402) with a CreditsError body was misclassified as permanent `auth` instead of the more actionable `credit_dead`.
- fix: `readiness.js`'s `probeOne` now classifies probe failures via `chain-machine.js`'s `classifyError` and calls `availability.markCreditDead` on `credit_dead` - previously bypassed chain-machine entirely, so a model the readiness prober itself proved billing-dead never got excluded until a real user request hit it.
- fix: `availability.js`'s latency penalty cap (10) was far lower than its success-streak bonus cap (30), letting a 50s-average model with a long success streak outrank a 2.2s-average model with fewer successes; raised to match.
- fix: `model_unhealthy` preCheck blocks were waited out for the full same-link retry budget (up to 10min) with no ETA, same as `sampler_backoff` (which has one); bounded to a separate, much shorter `MODEL_UNHEALTHY_WAIT_BUDGET_MS`.
- fix: added an in-flight, cross-request same-link-failure tracker so a second concurrent request no longer independently retries an already-storming lead model for minutes on its own.
- fix: added an overall chain wall-clock budget (`ACPTOAPI_CHAIN_WALL_CLOCK_BUDGET_MS`, default 90s) so per-link budgets that are each individually reasonable can no longer sum to an unbounded multi-minute hang across a 9-12 link auto chain.
- fix: `handleChat`'s `linkTimeout` now also falls back to `ACPTOAPI_CHAIN_LINK_TIMEOUT_MS` - previously only `ACPTOAPI_LINK_TIMEOUT_MS` reached the real `/v1/chat/completions` handler, so a deployment that set the other (more obviously-named) var had zero effect on real traffic.
- feat: `/v1/chat/completions` streaming is now real (chunk-by-chunk via `chain-machine.js`'s `runStream` + `sdk.stream()`) instead of buffering the full non-streamed response and synthesizing one fake SSE chunk at the end; non-streaming behavior is unchanged.
- fix: streaming responses now carry the same `X-Acptoapi-Served-Model`/`X-Acptoapi-Chain-Attempts`/`X-Acptoapi-Chain-Exhausted` diagnostic headers non-streaming responses already had.
- fix: `brand-catalog.js`'s `probeBrand` retries once on a probe timeout before giving up, and the four boot-time network tasks (model-probe-live, swe-bench-scores, brand-catalog, readiness warm-up) are now staggered (5s/6s/8s/11s) instead of three of them racing on the identical 5000ms timer, which was causing every brand-catalog probe to fail under contention on cold start.
- feat: `nvidia` (NVIDIA NIM) added to `FREE_TIER_PROVIDERS` - its entire catalog is free-tier reachable for any registered key, unlike a brand where a specific model can require paid credits on the same account.

## [1.0.213] - 2026-08-31

- fix: chain fallback now classifies HTTP 402 / CreditsError as a distinct `credit_dead` reason (chain-machine.js) instead of a bare `error` - a provider with no payment method now gets the same hard-exclusion treatment `auth`-dead providers already got, instead of occupying a chain slot on every request.
- fix: `brand-catalog.js` extends `isAuthDead`-style exclusion to credit-dead brands via new `isCreditDead`/`markCreditDead`/`isDead`; `buildAutoChain` excludes a credit-dead brand from the pool entirely. Closes a gap where a brand's `/models` catalog probe (e.g. `opencode-zen`) is billing-blind and 200s cleanly while every real chat call 402s - only a live chat-request failure can see this, so `markCreditDead` is wired from `chain-machine.js`'s `markProviderFailed`.
- fix: `xai-oauth.js` now detects a permanently-invalid `refresh_token` (HTTP 400 `invalid_grant`) and persists `refresh_dead: true` to `~/.acptoapi/xai-oauth.json`; `auto-chain.js`'s `xaiOauthLoggedIn()` excludes the provider once dead instead of re-attempting the same doomed refresh on every request and every readiness probe cycle. A fresh `--xai-oauth-login` clears the flag.
