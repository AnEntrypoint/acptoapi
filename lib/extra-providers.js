...
async function discoverOpenAI(parsed, apiKey, timeoutMs, fallbackModel) {
  const candidates = candidateChatURLs(parsed);
  const headers = {
    'Content-Type': 'application/json',
    'Authorization': `Bearer ${apiKey}`,
  };
  // Strict hosts (live-witnessed: api.moonshot.ai) answer 404 for ANY unknown
  // model id, so format discovery with the hardcoded gpt-4o-mini reads as
  // "no endpoint here" even when /v1/chat/completions is perfectly healthy.
  // When the caller pre-declared models for this entry, retry a fully-404
  // pass with the first declared model -- a 404 then genuinely means "no
  // such route", while any other status confirms the endpoint.
  const models = [FORMAT_DISCOVERY_OPENAI_MODEL];
  if (fallbackModel && !models.includes(fallbackModel)) models.push(fallbackModel);
  for (const model of models) {
    const body = { model, messages: [{ role: 'user', content: 'hi' }], max_tokens: 1 };
...
async function discoverAnthropic(parsed, apiKey, timeoutMs, fallbackModel) {
  ...
  // Same strict-host 404 problem as discoverOpenAI above.
  const models = [FORMAT_DISCOVERY_ANTHROPIC_MODEL];
  if (fallbackModel && !models.includes(fallbackModel)) models.push(fallbackModel);
  for (const model of models) {
...
async function probeEntry(entry, timeoutMs, modelProbeTimeoutMs) {
  ...
  const declaredModel = Array.isArray(entry.models) && entry.models.length > 0 ? entry.models[0] : null;
  const anthropicURL = await discoverAnthropic(parsed, entry.apiKey, timeoutMs, declaredModel);
  if (STAGGER_MS > 0) await new Promise(r => setTimeout(r, STAGGER_MS));
  const openaiURL = await discoverOpenAI(parsed, entry.apiKey, timeoutMs, declaredModel);
...
// Prefix is derived deterministically from the base URL (an 8-hex-char
// sha256 slice), not an incrementing in-memory counter. The counter reset to
// 0 on every process restart while prefixByBaseURL was never persisted, so
// the SAME configured base URL got re-registered as a freshly incrementing
// extra-0, extra-1, extra-2... on every restart -- live-witnessed: 274 stale
// zero-model entries (extra-0 through extra-273) accumulated in
// ~/.acptoapi/brand-catalog-cache.json for what was really one single
// endpoint, re-probed under a new name each time instead of reusing its
// prior cache entry. A hash-derived prefix is stable across restarts, so the
// same URL always maps to the same prefix and cache key.
function nextPrefix(baseURL) {
  if (baseURL && prefixByBaseURL.has(baseURL)) return prefixByBaseURL.get(baseURL);
  const hash = baseURL
    ? crypto.createHash('sha256').update(baseURL).digest('hex').slice(0, 8)
    : crypto.randomBytes(4).toString('hex');
  let p = `extra-${hash}`;
  // Collision with a real registered brand name is astronomically unlikely
  // for a hex hash, but guarded the same way the old counter loop was.
  let salt = 0;
  while (isBrand(p)) {
    salt++;
    p = `extra-${hash}${salt}`;
...
function registerOne(entry, probeResult) {
  ...
  // registerBrand's URL is resolved by resolveModel() and consumed
  // exclusively by the 'openai-compat' provider (lib/providers/openai.js),
  // which ALWAYS sends an OpenAI-shaped request body regardless of which
  // URL was registered. Preferring probeResult.anthropic here (the previous
  // order) pointed openai-compat's OpenAI-format body at a server's
  // Anthropic-format endpoint whenever BOTH formats were detected on the
  // same base URL (e.g. vLLM, which serves both /v1/chat/completions and
  // /v1/messages) -- live-witnessed: the resulting response came back in
  // Anthropic content[] shape, misread downstream as an empty OpenAI
  // choices[] result, so every call through this brand silently reported
  // "Empty response" even though the underlying model answered correctly.
  // openai must win whenever both are available; anthropic is the
  // fallback only for backends that speak solely the Anthropic wire format.
  const primaryURL = probeResult.openai || probeResult.anthropic;
...
// loadAndRegister does three unsafe-to-interleave things: unregisterAll()
// (briefly wipes every registered brand), a full read-modify-write cycle
// over the shared on-disk probe cache (each entry's own in-memory `cache`
// object is read once at the top and only saved back later -- a SECOND
// concurrent call reads the same stale disk state and can overwrite a
// first call's still-in-progress work with a smaller/incomplete result),
// and a genuinely slow multi-model probe pass (a fresh 25-model aggregator
// discovery is tens of seconds). Live-witnessed: casey's boot sequence
// alone (multiple independent status()/isReachable() checks, each capable
// of triggering its own loadAndRegister() call) raced two calls against
// the same cache file, and the SECOND (faster, 1-explicit-model-only) call
// won the final saveCache(), silently discarding the first call's
// still-in-flight 24-model auto-discovery merge -- exactly the "why isn't
// the extra provider being used" symptom this was chasing. A single
// process-wide in-flight promise serializes every caller onto the SAME
// underlying probe pass instead of racing separate ones.
let _inFlightLoad = null;
async function loadAndRegister(filePath) {
  if (_inFlightLoad) return _inFlightLoad;
  _inFlightLoad = _loadAndRegisterImpl(filePath).finally(() => { _inFlightLoad = null; });
  return _inFlightLoad;
}

async function _loadAndRegisterImpl(filePath) {
  ...
  for (let ei = 0; ei < rawEntries.length; ei++) {
    ...
    // entry.models (explicit models declared in extra-providers.txt) are
    // ADDITIVE to auto-discovery, never a replacement for it -- a model an
    // aggregator serves but omits from its own /v1/models listing (real,
    // live-witnessed: tokenhun.shadw.app's ep-gmv4flash works fine and is
    // NOT one of the 24 ids its own /v1/models enumerates) previously had no
    // way to be probed/registered at all once ANY explicit model was listed,
    // because the old gate treated a non-empty entry.models as "skip
    // auto-discovery entirely." Auto-discovery now always runs on a FRESH
    // probe (never replayed against a cache hit -- that would re-probe every
    // model on every loadAndRegister() pass, defeating the whole point of
    // the cache) whenever an OpenAI-compat chat URL was found, regardless of
    // whether explicit extra models were also configured for this entry.
    if (isFreshProbe && (probeResult.openai || probeResult.anthropic)) {
      ...
      if (chatURL) {
        ...
        const alreadyProbed = probeResult.models ? new Set(probeResult.models.keys()) : new Set();
        const toProbe = modelCandidates.filter(m => !alreadyProbed.has(m));
        if (toProbe.length > 0) {
          console.log(`[extra-providers] auto-discovered ${toProbe.length} model(s) for ${entry.baseURL}`);
          const discoveredResults = await probeModels(chatURL, entry.apiKey, toProbe, PROBE_TIMEOUT, STAGGER_MS);
  ...
  try {
    ...
    for (const entry of rawEntries) {
      ...
      if (cached.openai || cached.anthropic) {
        ...
        const prefix = nextPrefix(entry.baseURL);
        const envKey = generateEnvKey(prefix);
        // openai-first: see the matching comment in registerOne() above.
        const primaryURL = cached.openai || cached.anthropic;
        keyring.registerKey(envKey, entry.apiKey);
        registerBrand(prefix, { url: primaryURL, envKey });
