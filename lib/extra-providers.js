...
// STABLE per-baseURL prefix, live-witnessed bug this fixes: loadAndRegister
// calls unregisterAll() then re-registers on every call (each readiness
// re-probe cycle, see start() below), and nextPrefix() used to hand out a
// fresh incrementing prefix (extra-0, extra-1, extra-2, ...) EVERY single
// time regardless of whether it was the exact same configured endpoint as
// last time. availability.js tracks success/fail history keyed by the full
// "prefix/model" string -- a prefix that changes every probe cycle means the
// SAME real model looks like a brand-new, never-tested candidate on every
// cycle, permanently discarding its accumulated streak/score right as the
// periodic re-probe fix (this module's start()) was meant to make that
// history increasingly reliable over time. USER DIRECTIVE: "those models
// work... we must fix that." baseURL -> prefix is now a persistent Map, so
// the same endpoint always gets the same "extra-N" identity across every
// re-probe for the life of the process; only a genuinely NEW baseURL
// (added to extra-providers.txt, or a fresh process) consumes a new slot.
const _prefixByURL = new Map(); // baseURL -> prefix
function nextPrefix(baseURL) {
  if (baseURL && _prefixByURL.has(baseURL)) return _prefixByURL.get(baseURL);
  let p;
  while (true) {
    p = `extra-${_counter}`;
    _counter++;
  ...
  if (baseURL) _prefixByURL.set(baseURL, p);
  return p;
}
