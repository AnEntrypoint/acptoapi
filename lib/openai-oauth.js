...
// The Codex CLI and IDE extension share this cache. Prefer it over this
// package's legacy token store so an already-signed-in Codex user never has
// to complete a second device-login flow. ACPTOAPI_OPENAI_OAUTH_PATH remains
// an explicit escape hatch for a deliberately separate identity.
...
function readCredential() {
  // An explicit path always wins, including when it is currently empty: it is
  // how an operator opts out of the shared Codex-login behavior.
  ...
  // Backward compatibility for users who previously ran
  // --openai-oauth-login and have no file-backed Codex session.
  return { record: store.read(), source: 'acptoapi' };
}

function readRecord() { return readCredential().record; }

function writeCodexRecord(file, record) {
  // Codex owns auth.json. Only merge refreshed token fields and our refresh
  // timestamp, retaining Codex's auth_mode, account metadata, and any future
  // fields it adds. Atomic rename avoids a half-written shared credential.
...
function writeRecord(record, source = readCredential()) {
  ...
  if (source.source === 'codex') return writeCodexRecord(source.path, saved);
...
async function login({ onPrompt } = {}) {
  ...
  // Keep the explicit acptoapi login command backward-compatible: it writes
  // its own store instead of replacing the user's active Codex session.
...
function translateToResponses(body) {
  ...
  return {
    ...
    stream: true,
    include: [],
    // ChatGPT's Codex Responses endpoint currently rejects the public
    // Responses API max_output_tokens field. Keep the caller's output
    // limit local rather than turning every streamed request into a 400.
  };
}
