...
function spawnWindowsDaemon(name, attempt, desc, log) {
    // On Windows, use cmd.exe with /B flag, output redirection, and explicit
    // stdio handling to fully suppress console window creation.
    let cmdLine;
    const nulFd = fs.openSync('nul', 'w');

    if (attempt.shell) {
        // Shell command: route through cmd with /B flag and NUL redirection
        cmdLine = `${attempt.shell} > nul 2>&1`;
        const args = ['/B', '/C', cmdLine];
        const proc = spawn('cmd.exe', args, {
            stdio: ['ignore', nulFd, nulFd],
            detached: true,
            windowsHide: true
        });
        proc.on('exit', () => { try { fs.closeSync(nulFd); } catch {} });
        return proc;
    } else {
        // Direct command: build full command line with args and NUL redirection
        const argStr = (attempt.args || []).map(a => {
            return a.includes(' ') ? `"${a}"` : a;
        }).join(' ');
        cmdLine = `${attempt.command} ${argStr} > nul 2>&1`.trim();
        const args = ['/B', '/C', cmdLine];
        const proc = spawn('cmd.exe', args, {
            stdio: ['ignore', nulFd, nulFd],
            detached: true,
