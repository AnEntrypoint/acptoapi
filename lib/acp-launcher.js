...
function spawnWindowsDaemon(name, attempt, desc, log) {
    // Windows console-window suppression: spawn cmd.exe with /D /S /C (the
    // documented invisible-batch flags), windowsHide:true (CREATE_NO_WINDOW),
    // detached:true (DETACHED_PROCESS so the daemon doesn't inherit our
    // console group), windowsVerbatimArguments:true (Node passes args
    // verbatim — cmd.exe quoting rules apply, not Node's), and stdio fully
    // redirected to a file (not 'ignore', not 'pipe' — both can briefly
    // re-attach a console on .cmd shim chains like npx → npm → node).
    ...
    if (attempt.shell) {
        ...
        const args = (attempt.args || []).map(a => (a.includes(' ') ? `"${a}"` : a)).join(' ');
    ...
    // Wrap in double-quotes so cmd.exe /C treats the whole thing as one command
    // (matches the Node child_process internal pattern).
    const verbatim = `/D /S /C "${cmdStr}"`;
    ...
    return spawn('cmd.exe', [verbatim], spawnOpts);
}
