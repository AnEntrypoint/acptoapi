...
// NOTE: ACP daemon packages are not yet published to npm. Configurations below are placeholders
// for when implementations become available. Set <NAME>_ACP_CMD to enable custom ACP servers.
const CMDS = {
    kilo: {
        port: 4780,
        attempts: process.env.KILO_ACP_CMD
            ? [{ shell: process.env.KILO_ACP_CMD }]
            : [], // No published npm package yet; set KILO_ACP_CMD to use custom
    },
    opencode: {
        port: 4790,
        attempts: process.env.OPENCODE_ACP_CMD
            ? [{ shell: process.env.OPENCODE_ACP_CMD }]
            : [], // No published ACP server yet; set OPENCODE_ACP_CMD to use custom
    },
    'gemini-cli': {
        port: 4810,
        attempts: process.env.GEMINI_CLI_ACP_CMD
            ? [{ shell: process.env.GEMINI_CLI_ACP_CMD }]
            : [], // No published ACP server yet; set GEMINI_CLI_ACP_CMD to use custom
    },
    'qwen-code': {
        port: 4820,
        attempts: process.env.QWEN_CODE_ACP_CMD
            ? [{ shell: process.env.QWEN_CODE_ACP_CMD }]
            : [], // No published ACP server yet; set QWEN_CODE_ACP_CMD to use custom
    },
    'codex-cli': {
        port: 4830,
        attempts: process.env.CODEX_CLI_ACP_CMD
            ? [{ shell: process.env.CODEX_CLI_ACP_CMD }]
            : [], // No published ACP server yet; set CODEX_CLI_ACP_CMD to use custom
    },
    'copilot-cli': {
        port: 4840,
        attempts: process.env.COPILOT_CLI_ACP_CMD
            ? [{ shell: process.env.COPILOT_CLI_ACP_CMD }]
            : [], // No published ACP server yet; set COPILOT_CLI_ACP_CMD to use custom
    },
    'cline': {
        port: 4850,
        attempts: process.env.CLINE_ACP_CMD
            ? [{ shell: process.env.CLINE_ACP_CMD }]
            : [], // No published ACP server yet; set CLINE_ACP_CMD to use custom
    },
    'hermes-agent': {
        port: 4860,
        attempts: process.env.HERMES_ACP_CMD
            ? [{ shell: process.env.HERMES_ACP_CMD }]
            : [], // No published ACP server yet; set HERMES_ACP_CMD to use custom
    },
    'cursor-acp': {
        port: 4870,
        attempts: process.env.CURSOR_ACP_CMD
            ? [{ shell: process.env.CURSOR_ACP_CMD }]
            : [], // No published ACP server yet; set CURSOR_ACP_CMD to use custom
    },
    'codeium-cli': {
        port: 4880,
        attempts: process.env.CODEIUM_ACP_CMD
            ? [{ shell: process.env.CODEIUM_ACP_CMD }]
            : [], // No published ACP server yet; set CODEIUM_ACP_CMD to use custom
    },
    'acp-cli': {
        port: 4890,
        attempts: process.env.ACP_CLI_CMD
            ? [{ shell: process.env.ACP_CLI_CMD }]
            : [], // No published ACP server yet; set ACP_CLI_CMD to use custom
    },
};
...
async function spawnDaemon(name, log) {
    ...
    for (const attempt of cfg.attempts) {
        ...
        // Skip if command doesn't exist (for non-shell attempts)
        // Exception: always try 'bun x'/npx even if not in PATH, they might work
