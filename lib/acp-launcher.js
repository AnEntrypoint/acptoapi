...
function spawnWindowsDaemon(name, attempt, desc, log) {
    let proc;
    if (attempt.shell) {
        // For shell commands, route through PowerShell with hidden window
        proc = spawn('powershell.exe', ['-WindowStyle', 'Hidden', '-NoProfile', '-Command', attempt.shell], {
            stdio: 'ignore',
            detached: true,
            windowsHide: true,
            shell: false
        ...
        // This allows the child to outlive the parent without showing a console window
