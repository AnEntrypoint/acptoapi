...
// Probe bun once at module load  - if missing, all bunx attempts are skipped
// to avoid wasted ENOENT cycles on machines with only Node installed.
const HAS_BUN = commandExistsSync('bun');
const HAS_NPX = commandExistsSync('npx');

async function commandExists(cmd) {
    // npx ships with Node, bun is optional. Both cached at module load.
...
function spawnWindowsDaemon(name, attempt, desc, log) {
    ...
    //   child only. Descendants need CREATE_NO_WINDOW to stay invisible.
    // - CREATE_NO_WINDOW (0x08000000): every descendant inherits an
    //   invisible console  - canonical way to suppress conhost flashes
    //   through .cmd-shim chains (bun x npm-installed-tool, npx package).
    // We deliberately do NOT OR DETACHED_PROCESS (0x08): it conflicts with
    // CREATE_NO_WINDOW and causes console-subsystem descendants (bun.cmd ->
    // node.exe chain) to allocate a NEW visible console on first stdout
    // write because they inherit no console at all. Node's detached:true
    // already handles process-group separation; we don't need the Win32 flag.
...
async function ensureRunning({ names = ['kilo', 'opencode', 'qwen-code', 'codex-cli', 'copilot-cli', 'cline', 'hermes-agent', 'cursor-acp', 'codeium-cli', 'acp-cli'], log = () => {} } = {}) {
    // ACP daemon auto-spawn is opt-in via ACPTOAPI_ENABLE_ACP=1 (default: disabled)
...
function liveDaemons() {
    // Sync snapshot of which ACP daemons are believed to be up. Updated when
    // wrappers are successfully created or torn down. Consumers (auto-chain)
    // use this to filter unreachable daemons OUT of the auto fallback chain
    // so we never burn turns on dead links.
    const out = new Set();
    for (const name of Object.keys(WRAPPERS)) {
        if (WRAPPERS[name] && WRAPPERS[name].ready) out.add(name);
    }
    for (const name of Object.keys(STATE)) {
        if (STATE[name] && !STATE[name].killed) out.add(name);
    }
    return out;
}

module.exports = { ensureRunning, waitForReady, isUp, spawnDaemon, registerDaemon, CMDS, liveDaemons };
