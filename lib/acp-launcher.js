...
function spawnWindowsDaemon(name, attempt, desc, log) {
    ...
    if (attempt.shell) {
        // shell string — last resort, must still hide via the outer cmd.exe
    ...
    // If the resolved path is a real .exe, spawn it directly — no cmd.exe
    // shim, no console-window propagation. This is the clean path for bun,
    // node, and most installed CLIs on Windows.
    if (/\.exe$/i.test(resolved)) {
        return spawn(resolved, args, spawnOpts);
    }

    // Otherwise (.cmd shim or bare unix-style script: npx, kilo, opencode
    // when installed via npm without .exe), fall through cmd.exe. The
    // immediate cmd.exe is hidden via windowsHide:true; downstream .cmd
    // shims may still flash but at least the parent is suppressed.
