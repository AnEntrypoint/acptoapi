...
function spawnWindowsDaemon(name, attempt, desc, log) {
    const nullPath = path.join(os.tmpdir(), `.acptoapi-${name}-${Date.now()}.log`);
    const nullFd = fs.openSync(nullPath, 'w');

    let proc;
    if (attempt.shell) {
        proc = spawn('powershell.exe', [
            '-NoProfile', '-WindowStyle', 'Hidden',
            '-Command', `Start-Process -WindowStyle Hidden -NoNewWindow -FilePath "${attempt.shell}" -RedirectStandardOutput "${nullPath}" -RedirectStandardError "${nullPath}"`
        ], { stdio: ['ignore', nullFd, nullFd] });
    } else {
        const psCmd = `Start-Process -WindowStyle Hidden -NoNewWindow -FilePath "${attempt.command}" ${attempt.args ? `-ArgumentList ${JSON.stringify(attempt.args.join(','))}` : ''} -RedirectStandardOutput "${nullPath}" -RedirectStandardError "${nullPath}"`;
        proc = spawn('powershell.exe', ['-NoProfile', '-WindowStyle', 'Hidden', '-Command', psCmd], { stdio: ['ignore', nullFd, nullFd] });
    }

    proc.on('exit', () => {
        try { fs.closeSync(nullFd); } catch {}
        try { fs.unlinkSync(nullPath); } catch {}
    });
...
async function ensureRunning({ names = ['kilo', 'opencode', 'gemini-cli', 'qwen-code', 'codex-cli', 'copilot-cli', 'cline'], log = () => {} } = {}) {
    registerCleanupHandler();
    const status = {};
