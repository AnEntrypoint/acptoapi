...
function spawnWindowsDaemon(name, attempt, desc, log) {
    const stdio = getStdioDestFile();
    const stdioConfig = stdio ? ['ignore', stdio, stdio] : 'ignore';

    let proc;
    if (attempt.shell) {
        // For shell commands, route through cmd.exe with hidden window
        proc = spawn('cmd.exe', ['/c', attempt.shell], {
            stdio: stdioConfig,
            detached: true,
            windowsHide: true,
            shell: false
        ...
        // For command + args, spawn directly with detached mode and hidden window
        // Redirect stdio to temp file to prevent console window appearance
