...
async function sendMessage(backend, sessionId, text, modelId) {
  ...
  const timeoutMs = Number(process.env.ACP_MESSAGE_TIMEOUT_MS) || 180000;
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(new Error(`ACP /message timeout after ${timeoutMs}ms`)), timeoutMs);
  try {
...
async function listModels(backend, timeoutMs = 30000) {
  ...
  try {
    ...
    const acpModelObjects = Array.isArray(data?.models) ? data.models : Array.isArray(data) ? data : [];
    return acpModelObjects.map(m => (typeof m === 'string' ? m : m?.id || m?.modelID || m?.name)).filter(Boolean);
  } catch {
    return [];
  } finally {
    clearTimeout(t);
