...
async function chatCompletion(body, { timeoutMs, quiet, maxRetries } = {}) {
  ...
  // xAI subtracts max_tokens from remaining prompt budget; drop it here so
  // every chatCompletion caller (executeXaiOauthModel, readiness probes)
  // inherits the omit, not only the HTTP chain path. See shouldOmitMaxTokens.
