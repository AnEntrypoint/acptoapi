...
function convertInlineOrGenericImageSource(b) {
  if (!(b.inlineData || b.type === 'image')) return undefined;
...
function convertFileDataImageSource(b) {
  if (!b.fileData) return undefined;
  return { fileData: { mimeType: b.fileData.mimeType, fileUri: b.fileData.fileUri } };
}

function convertAnthropicStyleImageSource(b) {
  if (!(b.type === 'image' && b.source)) return undefined;
...
function convertImageBlock(b) {
  return convertInlineOrGenericImageSource(b)
    || convertFileDataImageSource(b)
    || convertAnthropicStyleImageSource(b)
    || null;
}
