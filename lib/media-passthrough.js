...
// acptoapi forwards the request to the configured upstream so callers have one
// place to send LLM/media traffic instead of importing each vendor SDK.
...
async function forwardJson({ req, res, json, target }) {
    const apiKey = require('./keyring').getKey(target.env);
    if (!apiKey) {
