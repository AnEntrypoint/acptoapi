...
// `model` is optional and defaults to '' so existing callers are unchanged; when
// a caller passes the requested model we echo it back, as the OpenAI contract
// expects (response.model should reflect the request, not an empty string). usage
// stays zeroed: the provider event stream does not carry token counts, so any
// non-zero number here would be a dishonest fabrication -- zeros plainly signal
// "not measured" rather than claiming a count we do not have.
function toResponse(events, model = '') {
  let text = '';
