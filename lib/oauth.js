...
const TOKEN_PATH = path.join(HOME, '.acptoapi', 'oauth-tokens.json');
const LEGACY_TOKEN_PATH = path.join(HOME, '.thebird', 'oauth-tokens.json');
...
function readTokens() {
  try { return JSON.parse(fs.readFileSync(TOKEN_PATH, 'utf8')); } catch {}
  try { return JSON.parse(fs.readFileSync(LEGACY_TOKEN_PATH, 'utf8')); } catch {}
  return null;
}
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(port, '127.0.0.1', () => {
      ...
      try {
        ...
        // Open URL in default browser without flashing a conhost window.
        // exec() via cmd-start pops a console on Windows; using spawn with
        // CREATE_NO_WINDOW on cmd.exe directly suppresses it.
        if (process.platform === 'win32') {
          spawn('cmd.exe', ['/D', '/S', '/C', 'start', '""', url.toString()], {
            detached: true,
            stdio: 'ignore',
            windowsHide: true,
          ...
          spawn(process.platform === 'darwin' ? 'open' : 'xdg-open', [url.toString()], {
            detached: true,
            stdio: 'ignore',
          }).unref();
        }
