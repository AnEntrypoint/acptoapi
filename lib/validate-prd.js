...
// Item 3: verify-acptoapi-startup
console.log('\nItem 3: verify-acptoapi-startup');
console.log('─'.repeat(55));

test('All 10 ACP daemons registered in launcher', () => {
  const launcher = require('./acp-launcher');
  const daemons = ['kilo', 'opencode', 'qwen-code', 'codex-cli',
...
test('All 10 ACP backends registered in client', () => {
...
if (failed === 0) {
  ...
  console.log('  3. verify-acptoapi-startup: server structure verified ✓');
  ...
  console.log('  • Start acptoapi.js and test endpoints');
