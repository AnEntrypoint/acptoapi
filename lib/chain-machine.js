...
// Live-witnessed: nvidia's own infra returned a 503 "Worker local total
// request limit reached" after 44 SECONDS -- against a chain whose overall
// caller-side timeout (casey's turn budget) was 120s, one slow hop alone
// could eat over a third of it, and combined with normal latency on the
// other hops the whole chain blew the caller's deadline with NO reply sent
// (this is casey's own no-fallback-text design working correctly -- the
// actual bug was upstream, a hop with no bound on how long it could run).
// 20s is long enough for any of this project's real configured providers
// under normal load (all confirmed sub-2s live) while still leaving room
// for several hops within a typical 60-120s caller budget. A caller that
// wants a different bound still can via link.timeout/opts.timeout, which
// take priority over this default exactly as before.
const DEFAULT_LINK_TIMEOUT_MS = 20000;
