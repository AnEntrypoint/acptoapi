...
function classifyError(err) {
  ...
  //
  // NOTE: xAI's Grok OAuth (SuperGrok / X Premium+) also surfaces an HTTP 403
  // with a similarly billing-shaped body (`code:
  // "personal-team-blocked:spending-limit"`, "You have run out of credits or
  // need a Grok subscription") -- deliberately NOT matched here. Unlike this
  // repo's `credit_dead` (a permanent per-model exclusion, e.g. a genuinely
  // missing payment method that never self-heals), xAI's spending-limit block
  // is a periodic usage cap that clears on its own (live-witnessed resetting
  // on the order of a day, account working on grok.com throughout) --
  // matching hermes-agent's own classification exactly (its
  // agent/credential_pool.py `_exhausted_ttl` docstring names this identical
  // xAI 403 explicitly as billing that still gets a bounded, retried
  // cooldown, never permanent removal). lib/xai-oauth.js tags this case
  // `err.code = 'RATE_LIMIT'` instead, so it lands on the `code === '
  // RATE_LIMIT'` branch below and self-heals via the normal sampler backoff.
  if (/creditserror|no payment method|insufficient credits|payment.?required/i.test(msg)) return 'credit_dead';
  // Explicit caller-tagged `err.code` is a more specific, intentional signal
  // than the generic status-code fallback below (e.g. xai-oauth.js tags
  // RATE_LIMIT on both its 429s and its billing-shaped 403s, since the latter
  // is a transient cooldown, not a dead credential) -- checked here, BEFORE
  // the status check, for the same reason the message-shape check above is
  // checked before it. Previously this `code` block sat after the status
  // check, so `code === 'CREDIT_DEAD'`/`code === 'AUTH'`/`code ===
  // 'RATE_LIMIT'` could never fire whenever `status` was already
  // 401/403/429 (the status check always won first) -- dead code for
  // exactly the callers it existed to serve, including the xai-oauth 403
  // case above: without this reorder, `status === 403` on the next line
  // would win before `code === 'RATE_LIMIT'` was ever reached.
...
async function waitForLeadLinkPrecheck(link, opts, isLeadLink, linkRetryStartedAt, chainDeadline, isLastLink) {
  ...
  //
  // The bare preCheck() below still had the identical defect for its own
  // sampler_backoff verdict specifically: sampler.isAvailable() is an
  // in-memory read with NO live HTTP attempt behind it, so a single-model
  // pinned request (no fallback candidate exists) could fail the whole
  // request purely off a stale backoff timer even though the provider might
  // already be healthy again (backoff windows are frequently shorter than
  // the time between the failure that set them and the next real request).
  // Witnessed live 2026-09-07: a harness pinned to xai-oauth/grok-4.6 got
  // "every chain link failed: xai-oauth/grok-4.6 (sampler_backoff)" while
  // direct, immediate curl calls to the exact same model succeeded --
  // proving the provider was live, only the cached backoff verdict was
  // stale. A single-model chain has nothing else to fall back to, so the
  // same "a real attempt can only be equal-or-better" reasoning above
  // applies here too: force one real attempt through instead of trusting
  // the cached verdict as final.
  ...
  while (true) {
    ...
    // waitMs can be a legitimate 0 (Math.max(0, nextRetryAt - now) above,
    // when the backoff window already passed) -- `||` would replace that
    // "retry right now" signal with the full step-max delay instead, same
    // bug class as the two timeout collapses above. `??` only.
  ...
  if (opts.modelHealth !== false) {
    try {
      ...
      if (peeked && peeked.effectiveFailStreak >= MIN_FAILSTREAK_TO_SKIP_MODEL && peeked.rank < 0) {
        // A model with prior success history (lastSuccessTs set) hit the same
        // permanent-lockout shape the single-flight claim below already fixes
        // for zero-success models: effectiveFailStreak decays on a 30-minute
        // half-life (see availability.js), so a burst of failures (a since-
        // fixed outage, an expired token later refreshed) keeps this gate
        // closed far longer than the real recovery -- and since preCheck
        // blocks BEFORE any HTTP attempt, the model can never record the one
        // real success that would reset failStreak on its own (witnessed
        // live: xai-oauth/grok-4.6 stuck at effectiveFailStreak~17 for 30+
        // minutes after credentials/model were independently confirmed
        // healthy via a direct out-of-band call). Reuse the same bounded
        // single-flight probe claim used below so a recovered-but-still-
        // decaying model gets one real attempt per claim window instead of
        // staying gated until decay alone clears it.
        const now = Date.now();
      ...
      // rank. This second single-flight claim covers the zero-success case
      // specifically (rank alone can't gate it the way it does above, since
      // score() has no success history to weigh against the fail penalty) --
      // a model still within its fresh-failure window already got skipped
      // above regardless of this claim, and a model WITH success history now
      // gets the same probe treatment via the block above instead of being
      // exempted from it.
...
async function* runStream(linksIn, opts, streamFn, registerRun) {
  ...
  // An empty pool (every provider filtered out -- no credentials, ACP daemons
  // disabled, sampler backoff, etc.) used to reach the state machine anyway,
  // which read `links[0]` as `undefined` and crashed several calls deep with
  // a cryptic "Cannot read properties of undefined (reading 'model')" instead
  // of a diagnosable error. Fail fast, here, with the actual cause.
  if (links.length === 0) {
  ...
  // Per-request override of the module-level SAME_LINK_RETRY_BUDGET_MS, same
  // shape as chainBudgetMs above. That module constant is sourced from
  // ACPTOAPI_SAME_LINK_RETRY_BUDGET_MS -- live-witnessed set to 45000 in
  // ~/.acptoapi/.env, tuned entirely for the MULTI-link case (many free-tier
  // fallback candidates, fail a flaky lead fast and move to the next one).
  // Applied unconditionally, that 45s window also gated same-link retries
  // for a single-link pinned chain with zero fallback candidates, where
  // failing fast just means failing -- witnessed live: a pinned
  // xai-oauth/grok-4.6 stream's first transient blip after 60s of otherwise
  // healthy streaming hit `elapsed(60022) >= budgetMs(45000)` and was
  // refused a retry outright, surfacing full chain exhaustion. server.js's
  // handleChat now passes a much larger (or explicitly configured) budget
  // for the single-link case via this override.
  ...
  while (true) {
    ...
    // `||` treats a deliberate 0 (server.js's linkTimeout: "single-link
    // chain, no fixed ceiling") the same as "absent", silently falling
    // through to DEFAULT_LINK_TIMEOUT_MS (120s) -- the exact ceiling the
    // single-link-chain fix existed to remove. Live-witnessed: a pinned
    // xai-oauth/grok-4.6 High-reasoning-effort turn kept hitting "every
    // chain link failed: xai-oauth/grok-4.6 (timeout)" even after
    // server.js's executeXaiOauthModel was fixed to forward timeoutMs:0
    // correctly (its own truthiness-vs-!=null bug) -- because THIS line
    // never let the 0 survive to reach that fixed call site at all. `??`
    // (nullish, not `||`) so only a genuinely absent link.timeout/opts.timeout
    // (undefined/null) falls through to the default; an explicit 0 stays 0.
    const timeout = link.timeout ?? opts.timeout ?? DEFAULT_LINK_TIMEOUT_MS;
    // Re-attach the resolved per-link timeout onto callOpts (stripped above
    // via `_to` so it never collides with an unrelated same-named field a
    // chatFn/streamFn might read differently) -- a chatFn like
    // server.js's executeXaiOauthModel needs the SAME ceiling this outer
    // Promise.race/withTimeout already enforces so it can bound its own raw
    // fetch with a real AbortSignal instead of leaving a stalled connection
    // running in the background after this loop stops waiting on it. See
    // xai-oauth.js's chatCompletion / XAI_OAUTH_DEFAULT_TIMEOUT_MS.
    ...
    while (true) {
      ...
      try {
        ...
        const elapsed = Date.now() - linkRetryStartedAt;
        if (!any && SAME_LINK_RETRY_REASONS.has(reason) && elapsed < sameLinkRetryBudgetMs && Date.now() < chainDeadline && sameReasonStreak < SAME_LINK_CONSECUTIVE_REASON_CAP) {
...
async function runChat(linksIn, opts, chatFn, registerRun) {
  await hydrateMatrix(opts);
  const links = reorderByMatrix(linksIn, opts);
  // See runStream's identical guard above: an empty pool must fail fast with
  // a diagnosable error, not crash deep in precheck logic on `links[0]`
  // being undefined.
  if (links.length === 0) {
    const err = new Error('No available chain links: every provider/model was filtered out (missing credentials, ACP daemons disabled, or sampler backoff) before dispatch was attempted.');
    err.code = 'NO_CHAIN_LINKS';
  ...
  while (true) {
    ...
    // (undefined/null) falls through to the default; an explicit 0 stays 0.
    const timeout = link.timeout ?? opts.timeout ?? DEFAULT_LINK_TIMEOUT_MS;
    // See runStream's identical comment above: re-attach the resolved
    // per-link timeout so chatFn (server.js's executeXaiOauthModel) can bound
    // its own raw fetch with a real AbortSignal, not just rely on this loop's
    // outer Promise.race to stop waiting on a connection that never aborts.
    const callOpts = { ...cleanOpts, ...link, model: link.model, timeout };
    let attempt = 0;
    let outcome = null;
    const linkRetryStartedAt = Date.now();
