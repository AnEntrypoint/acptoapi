...
const DEFAULT_LINK_TIMEOUT_MS = Number(process.env.ACPTOAPI_CHAIN_LINK_TIMEOUT_MS) || 10000;
...
const PROVIDER_LEVEL_HEALTH_REASONS = new Set(['error', 'timeout', 'rate_limit', 'auth', 'fetch_failed', 'empty']);
...
function markProviderFailed(model, reason, opts) {
  ...
  if (!provider) return;
  try {
    require('./sampler').markFailed(provider);
  } catch (e) {
...
function preCheck(link, opts) {
  ...
  if (opts.modelHealth !== false) {
    try {
      ...
      if (peeked && peeked.failStreak >= MIN_FAILSTREAK_TO_SKIP_MODEL && availability.score(link.model) < 0) {
