...
// distinct from SAME_LINK_RETRY_BUDGET_MS (per-link patience) -- that bound
// each INDIVIDUAL link, but nothing previously bounded the sum across a whole
...
// Lead-link preCheck only: a non-lead fallback has no "stay here" expectation.
// sampler_backoff on the lead is forced through so a stale in-memory backoff
// cannot fail a single-model request with zero live attempt.
function waitForLeadLinkPrecheck(link, opts, isLeadLink) {
  if (!isLeadLink) return null;
  // sampler.isAvailable() is an in-memory read with no live HTTP attempt, so
  // a single-model pinned request could fail the whole request off a stale
  // backoff timer. Force one real attempt instead of trusting the cached
  // verdict.
