...
const BUILT_IN = {
  ...
  openrouter: {
    options: {},
    request(req, opts) {
      const headers = { 'HTTP-Referer': 'https://github.com/AnEntrypoint/acptoapi', 'X-Title': 'acptoapi', ...(opts || {}).headers };
...
function resolveTransformers(useList, customMap) {
  ...
  return useList.map(entry => {
    ...
    if (!t) { console.warn('[acptoapi] unknown transformer:', name); return null; }
...
module.exports = { resolveTransformers, applyRequestTransformers };
