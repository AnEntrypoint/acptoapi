...
//
// HARDCODED_AGGREGATOR_PREFIXES mirrors chain-machine.js's own set: a
// hardcoded brand-catalog aggregator (openrouter re-exports many independent
// third-party brands under nested model ids) is exempted the same way a
// user-registered multi-model extra-provider is, so an unconfigured/failing
// candidate this prober probes under that prefix (e.g. openrouter/x-ai/grok-4.6
// with no credits) never trips sampler.markFailed for the whole prefix and
// backs off an unrelated, healthy sibling like openrouter/ox-alpha.
...
// xai-oauth carries its bearer token in an on-disk OAuth store (lib/xai-oauth.js),
// not a keyring env key -- sdk.js's generic openai-compat resolution has no
// knowledge of that store and sends `Authorization: Bearer undefined` for
// this brand, so every probe through the generic path fails with a real-
// looking "Incorrect API key provided" regardless of actual token health.
// Route this one brand's probe through the same credential-aware
// xaiOauth.chatCompletion real traffic already uses (server.js's
// executeXaiOauthModel) so a probe result reflects the real auth state
// instead of a structural false negative that poisons availability.js's
// failStreak on every single pass.
...
async function probeOne(model) {
  ...
  try {
    ...
    // A 402/CreditsError discovered only by the readiness probe (never sent
    // real user traffic) previously never reached availability.markCreditDead
    // -- that only fired from chain-machine.js's own real-request failure
    // path, which probeOne bypasses entirely (it calls sdk.chat directly).
    // Without this, a model the prober itself proves permanently billing-dead
    // stayed eligible for real chain-lead promotion until an actual user
    // request happened to hit it and fail the same way.
    // A rate_limit probe failure (e.g. xai-oauth's spending-limit 403, tagged
    // with a real e.retryAfter) previously reached recordFailure() with no
    // second argument, which unconditionally overwrites retryNotBeforeTs to
    // null (availability.js's recordFailure: `retryAfterMs` not a number ->
    // clear it) -- readiness probes run every ACPTOAPI_READINESS_INTERVAL_MS
    // (default ~2min) against the SAME model, so a real chain-routed request
    // that had just set a genuine 15-minute "don't retry before this" deadline
    // via retryAfterMsFor had it silently wiped by the very next probe,
    // defeating isRetryDeadlineActive()'s whole purpose for any actively-
    // probed model. Compute the same classification + retryAfterMsFor
    // chain-machine.js's own real-traffic failure path already uses, so a
    // probe respects the provider's stated deadline exactly like real
    // traffic does instead of clobbering it.
    let reason;
    try {
