...
function recordRun(actor, meta) {
  const entry = {
    ...
    resolvedLinksWithRank: meta?.resolvedLinksWithRank || [],
    attempted: [],
    finalModel: null,
  };
...
// Direct, actor-free run registration for callers that don't drive the
// xstate FSM (e.g. handleAnthropicMessages's hand-rolled candidate loop) but
// still want their fallback traversal visible in GET /v1/runs. Appends a
// fully-formed entry in one call rather than subscribing to state-transition
// events like recordRun does.
function recordRunDirect(meta) {
  const entry = {
    ts: Date.now(),
    ...
    finalModel: meta?.finalModel || meta?.servedBy || null,
  };
  RUN_HISTORY.push(entry);
  if (RUN_HISTORY.length > HISTORY_MAX) RUN_HISTORY.shift();
  return entry;
...
module.exports = { chain, fallback, resolveNamedChain, listNamedChains, getRunHistory, recordRunDirect };
