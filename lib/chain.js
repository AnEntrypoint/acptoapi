'use strict';
const { runStream, runChat, normalizeLink } = require('./chain-machine');
const { loadConfig } = require('./config');

const RUN_HISTORY = [];
const HISTORY_MAX = 50;

function recordRun(actor) {
  const entry = { startedAt: Date.now(), state: 'running', history: [], servedBy: null, finishedAt: null };
