    ...
    if (provider === 'anthropic') {
      ...
      if (brand) {
        ...
        if (key) {
          ...
          const probeModel = (KNOWN[provider] && KNOWN[provider][0]) || 'test';
...
function isAvailable(provider) {
    ...
    if (provider === 'anthropic') return keyring.hasAnyKey('ANTHROPIC_API_KEY');
    if (provider === 'google') return keyring.hasAnyKey('GEMINI_API_KEY');
    if (provider === 'gemini') return keyring.hasAnyKey('GEMINI_API_KEY');
...
async function getAvailableModelsLive({ log = () => {}, force = false } = {}) {
    ...
    for (const [provider, models] of Object.entries(KNOWN)) {
        ...
        for (const model of models) {
            ...
            const rawScore = getModelScore(fullId);
            const score = typeof rawScore === 'number' ? rawScore : null;
            if (score === null) log(`[known] ${fullId}: no swebench score (included, sorted last)`);
    ...
    const direct = out.filter(m => m.bucket === 'direct').sort(byScoreDesc);
    const acp = out.filter(m => m.bucket === 'acp').sort(byScoreDesc);
...
function getAvailableModels({ log = () => {} } = {}) {
    ...
    direct.sort(byScoreDesc);
    acp.sort(byScoreDesc);
...
module.exports = { getAvailableModels, getAvailableModelsLive, buildChainFromModels, isAvailable, probeProvider, clearProbeCache, loadProbeCache };
