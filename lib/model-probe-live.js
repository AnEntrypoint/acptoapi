...
const CACHE_TTL_MS = Number(process.env.ACPTOAPI_PROBE_TTL_MS || 10 * 60 * 1000);
...
const DISK_CACHE_PATH = process.env.ACPTOAPI_PROBE_CACHE_PATH
    || path.join(os.homedir(), '.acptoapi', 'probe-cache.json');
...
function loadDiskCache() {
    try {
        ...
        const j = JSON.parse(fs.readFileSync(DISK_CACHE_PATH, 'utf8'));
        if (!j || !Array.isArray(j.results) || typeof j.ts !== 'number') return null;
        return j;
    } catch { return null; }
}
...
function writeDiskCache(entry) {
    try {
        ...
        fs.writeFileSync(DISK_CACHE_PATH, JSON.stringify(entry, null, 2));
    } catch {}
}
...
async function probeAllProviders({ providers, log = () => {} } = {}) {
    ...
    if (providers) list = providers;
        ...
        // Always include direct providers beyond the brand list when keys exist or service runs locally.
        const direct = [];
        if (process.env.ANTHROPIC_API_KEY) direct.push('anthropic');
        if (process.env.GEMINI_API_KEY) direct.push('gemini');
        if (process.env.OLLAMA_URL || process.env.ACPTOAPI_PROBE_OLLAMA === '1') direct.push('ollama');
        list = [...direct, ...supported.filter(p => { try { return !!pickEnvKey(p); } catch { return false; } })];
    }
    log(`[probe] enumerating models for ${list.length} provider(s): ${list.join(', ')}`);

    // Known models for providers that don't expose /models in a usable way
    const KNOWN = {
    ...
    const listings = await Promise.all(list.map(async provider => {
        const apiKey = pickEnvKey(provider);
        if (KNOWN[provider]) {
            log(`[probe] ${provider}: using known model list (${KNOWN[provider].length})`);
            return { provider, models: KNOWN[provider] };
        }
...
async function getOrRefresh({ force = false, log = () => {} } = {}) {
    ...
    _inFlight = (async () => {
        try {
            ...
            writeDiskCache(_cache);
            log(`[probe] wrote ${results.length} working models to ${DISK_CACHE_PATH}`);
            return results;
...
function buildChainFromProbe(results, { localFallbacks = ['kilo/openrouter/free', 'opencode/minimax-m2.5-free'] } = {}) {
