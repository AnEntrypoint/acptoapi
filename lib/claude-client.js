...
async function* streamClaude({ prompt, model, systemPrompt, bin = 'claude', signal, tools = '', permissionMode = 'default', cwd }) {
  const spawnCwd = cwd || os.tmpdir();
  const args = [
    '-p', prompt,
    ...
    '--setting-sources', '',
    '--permission-mode', tools ? 'bypassPermissions' : permissionMode,
    '--model', model,
  ];
