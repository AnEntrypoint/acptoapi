...
async function handleChat(req, res, backends) {
  ...
  if (body.model && process.env.ACPTOAPI_DISABLE_CHAIN !== '1') {
    ...
    try {
      ...
      return json(res, 200, { id: rid, object: 'chat.completion', created: Math.floor(Date.now()/1000), model: mdl, choices: [{ index: 0, message: { role: 'assistant', content }, finish_reason: 'stop' }], usage: { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 } });
    }
  }
