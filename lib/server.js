...
const DEBUG_LOG = process.env.AGENTAPI_DEBUG_LOG === '1';

const ANTHROPIC_ROUTES = [
  { match: 'model starts with claude/', provider: 'claude', note: 'routes to Claude Code CLI' },
...
async function listModels(claudeBin, queuesProvider) {
...
async function handleClaudeChat(req, res, body, claudeBin) {
  ...
  const cwdHeader = req.headers['x-cwd'];
  const cwd = cwdHeader && cwdHeader.trim() ? cwdHeader.trim() : undefined;
  ...
  try {
    for await (const ev of streamClaude({ prompt: convo, model, systemPrompt, bin: claudeBin, signal: ctrl.signal, tools: hasTools ? 'default' : '', cwd })) {
...
async function handleBrandChat(req, res, brandName, body) {
  ...
  console.log(`[acptoapi] /v1/chat/completions provider=${brandName} model=${upstreamModel} stream=${body.stream === true}`);
...
async function handleChat(req, res, backends, claudeBin) {
  const body = await readBody(req);
  if (isClaudeModel(body.model)) { console.log(`[acptoapi] /v1/chat/completions provider=claude model=${body.model} stream=${body.stream === true}`); return handleClaudeChat(req, res, body, claudeBin); }
  const sub = splitBrandModel(body.model);
...
async function handleAnthropicMessages(req, res, backends, claudeBin) {
  ...
  const isBareClaudeName = typeof body.model === 'string'
    ...
    && !body.model.includes('/');
  let preferredClaudeLink = null;
  if (isBareClaudeName && process.env.ANTHROPIC_API_KEY) {
    body.model = 'anthropic/' + body.model;
  } else if (isBareClaudeName) {
    body.originalModel = body.model;
    const m = /claude-(haiku|sonnet|opus)/i.exec(body.model);
    preferredClaudeLink = m ? `claude/${m[1].toLowerCase()}` : 'claude/sonnet';
    body.model = 'auto';
  }
  // Named chain: caller asks for "smart", "fast", "free", "reasoning", "local", or any custom name.
  // Unrecognized name falls through to default behavior.
  ...
  if (namedLinks && namedLinks.length) {
    console.log(`[acptoapi] using named chain '${body.model}' (${namedLinks.length} links)`);
    body.model = 'auto';
  }

  // Build fallback queue: requested model first (unless 'auto'), then all available providers from .env
  const target = body.model && body.model !== 'auto' ? body.model : undefined;
  let queueLinks;
  if (namedLinks) {
    queueLinks = namedLinks;
  } else {
  const probeLib = require('./model-probe-live');
  const explicitLive = process.env.ACPTOAPI_LIVE_PROBE === '1' || req.headers['x-live-probe'] === '1';
  // Automatic live use whenever the probe cache is fresh (filled in background on startup).
  const wantLive = explicitLive || probeLib.isFresh();
  if (wantLive) {
    try {
      const probed = await probeLib.getOrRefresh({ log: m => console.log(m) });
      queueLinks = probeLib.buildChainFromProbe(probed);
      if (target) queueLinks.unshift({ model: target, fallbackOn: ['error','rate_limit','timeout','empty'] });
      console.log(`[acptoapi] live chain built from ${probed.length} working models`);
    } catch (e) {
      console.log(`[acptoapi] live probe failed (${e.message}), falling back to static chain`);
      queueLinks = buildAutoChain(target);
    }
  } else {
    // No fresh probe yet — kick one off in background so future requests use live.
    probeLib.kickoff(m => console.log(m));
    queueLinks = buildAutoChain(target);
  }
  }
  if (preferredClaudeLink) {
    // Replace any default claude/* link with the variant the caller requested
    const idx = queueLinks.findIndex(l => /^claude\//.test(l.model));
    if (idx >= 0) queueLinks[idx] = { ...queueLinks[idx], model: preferredClaudeLink };
    else queueLinks.push({ model: preferredClaudeLink, fallbackOn: ['error','rate_limit','timeout','empty'] });
  }
  ...
  function buildOpts(modelStr) {
    ...
    if (localProvider === 'brand') {
      ...
      const apiKeyVal = process.env[brand.envKey];
    ...
    if (effectiveProvider === 'claude') baseOpts.claudeBin = claudeBin;
...
function attachTerminalWs(server) {
  ...
  server.on('upgrade', (req, socket, head) => {
    ...
    if (!m) return;
    const sid = m[1];
    const requireAuth = process.env.AGENTAPI_API_KEY;
    if (requireAuth) {
...
function createServer({ port = 4800, backends = {}, claudeBin = 'claude', queuesProvider = null } = {}) {
  const requireAuth = process.env.AGENTAPI_API_KEY;
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      const url = new URL(req.url, 'http://x');
      metrics.inc('agentapi_requests_total', { path: url.pathname, method: req.method });
      res.on('finish', () => metrics.observe('agentapi_request_duration_ms', Date.now() - t0, { path: url.pathname }));
      ...
      if (requireAuth && !isPublic) {
        ...
        if (key !== requireAuth) { metrics.inc('agentapi_auth_failures_total'); return json(res, 401, { error: { message: 'Invalid API key' } }); }
      }
      if (url.pathname === '/metrics' && req.method === 'GET') {
        res.writeHead(200, { 'Content-Type': 'text/plain; version=0.0.4' });
        return res.end(metrics.render());
      }
      if (url.pathname === '/v1/models' && req.method === 'GET') return json(res, 200, await listModels(claudeBin, queuesProvider));
      ...
      if (url.pathname === '/debug/probe-live' && req.method === 'GET') {
        const { getOrRefresh, buildChainFromProbe } = require('./model-probe-live');
        const logs = [];
        const force = url.searchParams.get('force') === '1';
        const results = await getOrRefresh({ force, log: m => logs.push(m) });
        return json(res, 200, { results, chain: buildChainFromProbe(results).map(l => l.model), logs });
      }
      ...
      if (url.pathname === '/debug/anthropic' && req.method === 'GET') {
        const envVars = {};
        for (const key of ['NVIDIA_API_KEY', 'OPENAI_API_KEY', 'OPENAI_API_BASE', 'ANTHROPIC_API_KEY', 'GEMINI_API_KEY', 'AGENTAPI_API_KEY', 'AGENTAPI_DEBUG_LOG']) {
  ...
  return new Promise((resolve, reject) => {
    server.listen(port, () => {
      ...
      console.log(`agentapi listening http://localhost:${actual}`);
      // Kick off model probe in the background so the first /v1/messages
      // hit can use the live chain without waiting on a cold probe.
      if (process.env.ACPTOAPI_DISABLE_PROBE !== '1') {
        try { require('./model-probe-live').kickoff(m => console.log(m)); } catch {}
      }
      // Auto-launch ACP daemons (kilo, opencode) if not already running so
      // they're available as fallbacks before the claude CLI last-resort.
      if (process.env.ACPTOAPI_DISABLE_ACP_AUTOLAUNCH !== '1') {
        try { require('./acp-launcher').ensureRunning({ log: m => console.log(m) }); } catch {}
      }
