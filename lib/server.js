...
const { buildAutoChain, getOrder } = require('./auto-chain');
...
function buildModelProbes() {
  ...
  for (const m of models) {
    ...
    if (['kilo', 'opencode', 'gemini-cli', 'qwen-code', 'codex-cli', 'copilot-cli', 'cline', 'hermes-agent', 'cursor-acp', 'codeium-cli', 'acp-cli'].includes(m.provider)) {
...
async function listModels(queuesProvider) {
  ...
  for (const r of acp) {
    ...
    for (const m of MODEL_CATALOG[r.prefix]) {
      ...
      seen.add(id);
      const entry = { id, object: 'model', owned_by: r.prefix, created };
  ...
  if (process.env.NVIDIA_API_KEY) {
    try {
      const nvidiaModels = await discoverNvidiaModels(process.env.NVIDIA_API_KEY);
...
async function handleBrandChat(req, res, brandName, body) {
  ...
  if (allKeys.length === 0) return json(res, 401, { error: { message: `Missing ${envKey} env var for brand '${brandName}'` } });
...
async function ollamaEmbed(url, model, input) {
  // Ollama's embed endpoint is /api/embeddings with body {model, prompt} for
  // a single string OR {model, input: [...]} on newer versions. It returns
  // {embedding: [...]} for single OR {embeddings: [[...], ...]} for batch.
  // Translate to OpenAI's /v1/embeddings response shape so downstream
  // consumers (rs-plugkit host_vec_embed, etc) don't need to know the source.
  ...
  for (let i = 0; i < inputs.length; i++) {
    ...
    if (!r.ok) {
      const text = await r.text();
      const err = new Error(`ollama embed failed: ${r.status} ${text.slice(0, 200)}`);
      err.status = r.status;
    ...
    if (!j || !Array.isArray(j.embedding)) {
      throw new Error(`ollama embed returned unexpected shape: ${JSON.stringify(j).slice(0, 200)}`);
    }
    data.push({ object: 'embedding', embedding: j.embedding, index: i });
    if (typeof j.prompt_eval_count === 'number') totalTokens += j.prompt_eval_count;
  }
  return {
    object: 'list',
    data,
...
function pickEmbeddingBrandWithCredentials() {
  // First-pass: any cloud brand whose key is configured wins. ollama is the
  // last-resort local fallback so we don't force a localhost dependency on
  // users who DO have cloud keys.
  for (const name of listEmbeddingBrands()) {
    if (name === 'ollama') continue;
    const b = getEmbeddingBrand(name);
    if (!b || !b.envKey) continue;
    const keys = keyring.getKeys(b.envKey);
    if (keys.length > 0) return { name, brand: b };
  }
  return { name: 'ollama', brand: getEmbeddingBrand('ollama') };
}

async function handleEmbeddings(req, res) {
  const body = await readBody(req);
  const sub = splitBrandModel(body.model);
  let brandName = sub ? sub.prefix : null;
  let upstreamModel = sub ? sub.model : body.model;
  // No explicit brand prefix → autoselect: cloud brand with a key, else
  // fall through to ollama (local, no key required). This is the default
  // path for downstream consumers that just say "embed this string" without
  // caring where the vector comes from.
  if (!brandName) {
    const picked = pickEmbeddingBrandWithCredentials();
    brandName = picked.name;
    if (brandName === 'ollama' && (!upstreamModel || upstreamModel === body.model)) {
      // No cloud key AND no model specified → use ollama's default model.
      upstreamModel = picked.brand && picked.brand.defaultModel ? picked.brand.defaultModel : 'nomic-embed-text';
    }
  }
  ...
  // Local-brand path (ollama, future: kilo/opencode if they expose embed).
  // No bearer auth, different request/response shape.
  if (brand.local) {
    try {
      const modelToUse = upstreamModel || (brand.defaultModel || 'nomic-embed-text');
      const result = await ollamaEmbed(brand.url, modelToUse, body.input);
      res.writeHead(200, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Private-Network': 'true' });
      res.end(JSON.stringify(result));
    } catch (e) {
      const status = e.status || 502;
      json(res, status, { error: { message: e.message, brand: brandName, hint: 'ensure ollama is running at OLLAMA_URL (default http://localhost:11434) and the model is pulled (e.g. `ollama pull nomic-embed-text`)' } });
    }
    return;
  }
  // Cloud-brand path — requires API key, OpenAI-compatible request/response.
  const allKeys = keyring.getKeys(brand.envKey);
  if (allKeys.length === 0) return json(res, 401, { error: { message: `Missing ${brand.envKey}` } });
  try {
    const usable = keyring.listUsable(brand.envKey);
    const tryKeys = usable.length > 0 ? usable : [keyring.getKey(brand.envKey)];
    let r;
    for (let i = 0; i < tryKeys.length; i++) {
      const apiKey = tryKeys[i];
      r = await fetch(brand.url, {
      ...
      const reason = keyring.classify(r.status);
      if ((reason === 'auth' || reason === 'rate_limit') && i < tryKeys.length - 1) {
        keyring.markKeyFailed(brand.envKey, apiKey, reason);
        continue;
      }
      if (reason === 'auth' || reason === 'rate_limit') keyring.markKeyFailed(brand.envKey, apiKey, reason);
      else if (r.ok) keyring.markKeyOk(brand.envKey, apiKey);
      break;
    }
    const text = await r.text();
...
async function handleChat(req, res, backends) {
  ...
  if (!split) {
    ...
    return json(res, 400, { error: { message: `Unknown model '${body.model || ''}'. Use <brand>/<model> (brands: ${brandNames}) or <acp-agent>/<model> (acp: ${acpPrefixes}).` } });
  }
  ...
  try {
    ...
    if (!finished) {
      ...
      if (stream) { sse(res, { error: { message: msg } }); res.end(); }
      else json(res, 504, { error: { message: msg } });
...
async function handleAnthropicMessages(req, res, backends) {
  ...
  // Try to resolve named queue; if not found, use default queue from curated models
  ...
  if (requestedQueueName) {
    ...
    if (namedLinks && namedLinks.length) {
      console.log(`[acptoapi] using named queue '${requestedQueueName}' (${namedLinks.length} links)`);
      queueLinks = namedLinks;
    }
  }

  // If no named queue found or requested, build default queue from curated models
  if (!queueLinks) {
    const probeLib = require('./model-probe-live');
    const models = probeLib.getAvailableModels({ log: m => console.log(m) });
    queueLinks = probeLib.buildChainFromModels(models);
    console.log(`[acptoapi] using default queue built from ${models.length} curated models`);
    if (models.length === 0) {
      console.log(`[acptoapi] no curated models available, falling back to static chain`);
      queueLinks = buildAutoChain();
    }
  }
  if (queueLinks.length === 0) {
    return json(res, 503, { error: { type: 'api_error', message: 'No providers configured. Set at least one of: ANTHROPIC_API_KEY, GEMINI_API_KEY, GROQ_API_KEY, NVIDIA_API_KEY, etc. in .env' } });
  }
  ...
  if (!authHeader) { logAnthropic({ action: 'auth_fail', reason: 'missing auth header' }); return json(res, 401, { error: { type: 'authentication_error', message: 'Missing authentication' } }); }
  ...
  function buildOpts(modelStr) {
    ...
    const opts = effectiveProvider === 'openai-compat' || effectiveProvider === 'nvidia'
      ? {
          ...
          apiKey: localProvider === 'brand' ? lBrandApiKey : (process.env.OPENAI_API_KEY || process.env.NVIDIA_API_KEY),
          streamGuard: { chunkTimeoutMs: Number(process.env.OPENAI_STREAM_CHUNK_TIMEOUT_MS || 120000) },
          body: {
            model: localBody.model,
            messages: localBody.messages,
  ...
  if (streaming) {
    ...
    for (const candidate of queue) {
      ...
      catch (e) { tried.push({ model: candidate, error: e.message }); lastError = e; continue; }
      let anyData = false;
      let buffered = '';
      try {
        ...
        if (!anyData) { console.log(`[acptoapi] stream empty provider=${route.provider} model=${route.model}, falling back`); lastError = new Error(`Empty stream from ${candidate}`); tried.push({ model: candidate, error: 'empty' }); continue; }
        served = true;
        ...
        lastError = e;
        tried.push({ model: candidate, error: e.message });
        logAnthropic({ action: 'stream_fallback', from: candidate, error: e.message });
        continue;
      }
    }
    if (!served) {
      res.write(`event: error\ndata: ${JSON.stringify({ type: 'error', error: { type: 'api_error', message: `All ${tried.length} providers failed. Last: ${lastError ? lastError.message : 'unknown'}`, tried } })}\n\n`);
    }
    clearInterval(ping);
    res.end();
  } else {
    ...
    for (const candidate of queue) {
      ...
      try {
        ...
        if (!isValidAnthropicMessageResponse(result) || result.stop_reason === 'error' || result.content.length === 0) {
          ...
          tried.push({ model: candidate, error: reason });
    ...
    return json(res, 502, { error: { type: 'api_error', message: `All ${tried.length} providers failed. Last: ${lastError ? lastError.message : 'unknown'}`, tried } });
  }
}
...
function createServer({ port = 4800, backends = {}, queuesProvider = null } = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (url.pathname === '/v1/sampler/status' && req.method === 'GET') return json(res, 200, { status: sampler.getStatus() });
      ...
      if (url.pathname === '/v1/chains' && req.method === 'POST') {
        ...
        if (!body || typeof body.name !== 'string' || !Array.isArray(body.links) || !body.links.length) {
          return json(res, 400, { error: { message: 'expected {name: string, links: [<model>, ...]}' } });
        }
        try {
      ...
      if (url.pathname === '/v1/chains' && req.method === 'DELETE') {
        ...
        return json(res, ok ? 200 : 404, { ok, name });
      }
      ...
      if (url.pathname === '/debug/probe-live' && req.method === 'GET') {
        const { getAvailableModels, buildChainFromModels } = require('./model-probe-live');
        const logs = [];
        const models = getAvailableModels({ log: m => logs.push(m) });
      ...
      // Media passthrough endpoints — acptoapi forwards to upstream so freddie/thebird
      ...
      if (url.pathname === '/v1/images/generations' && req.method === 'POST') {
        ...
        if (!target) return json(res, 400, { error: { message: `unknown image provider: ${provider}` } });
      ...
      if (url.pathname === '/v1/audio/speech' && req.method === 'POST') {
        ...
        if (!target) return json(res, 400, { error: { message: `unknown speech provider: ${provider}` } });
        return forwardJson({ req, res, json: body, target });
      }
      ...
      if (url.pathname === '/v1/messages' && req.method === 'POST') return handleAnthropicMessages(req, res, backends);
  ...
  return new Promise((resolve, reject) => {
    server.listen(port, () => {
      ...
      if (process.env.ACPTOAPI_DISABLE_ACP_AUTOLAUNCH !== '1') {
        try {
