...
async function handleAnthropicMessages(req, res, backends) {
  ...
  if (streaming) {
    ...
    for (const candidate of queue) {
      ...
      try {
        ...
        try {
          ...
          recordRunDirect({ requestedModel: body.model, resolvedLinks: queue, servedBy: route.model, finalModel: route.model, startedAt, attempted: [...tried, { model: route.model, ok: true, reason: null }] });
        } catch {}
    ...
    if (!served) {
      ...
      try {
        const { recordRunDirect } = require('./chain');
        recordRunDirect({ requestedModel: body.model, resolvedLinks: queue, servedBy: null, finalModel: null, startedAt, attempted: tried });
      } catch {}
    }
    clearInterval(ping);
    res.end();
