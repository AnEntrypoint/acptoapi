...
const { buildAutoChain, buildAutoChainLive, getOrder, hasProvider } = require('./auto-chain');
  ...
  if (streaming) {
    ...
    for (const candidate of queue) {
      ...
      try {
        ...
        if (!anyData) { console.log(`[acptoapi] stream empty provider=${route.provider} model=${route.model}, falling back`); lastError = new Error(`Empty stream from ${candidate}`); tried.push({ model: candidate, error: 'empty', reason: 'empty' }); continue; }
        served = true;
