...
async function executeForModel(opts, backends) {
  ...
  const split = splitModel(opts.model);
  if (!split) { const e = new Error(`Unknown model '${opts.model}'`); e.status = 400; throw e; }
  return executeAcpModel(split.prefix, opts, backends);
}
...
async function handleChat(req, res, backends) {
  ...
  if (!wantsStream && body.model && process.env.ACPTOAPI_DISABLE_CHAIN !== '1') {
    try {
      const { runChat } = require('./chain-machine');
      const autoLinks = buildAutoChain(body.model);
