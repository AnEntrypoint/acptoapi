...
function buildModelProbes() {
  ...
  for (const m of models) {
    ...
    if (['kilo', 'opencode', 'gemini-cli', 'qwen-code', 'codex-cli', 'copilot-cli', 'cline', 'hermes-agent', 'cursor-acp', 'codeium-cli', 'acp-cli'].includes(m.provider)) {
      ...
      // For other providers (anthropic, google, etc.), skip for now - mark as OK since we check env keys in isAvailable
      probes.push({
        provider: m.provider,
        call: async () => {
          // Lightweight check: just verify env key is still set
          // Full 1-token probes happen via handleAnthropicMessages when requests arrive
        }
      });
    }
  }
