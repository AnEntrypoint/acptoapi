  ...
  if (body.model && process.env.ACPTOAPI_DISABLE_CHAIN !== '1') {
    ...
    try {
      ...
      if (cacheWrap.hit === 'hit' || cacheWrap.hit === 'dedupe') {
        ...
        console.log(`[acptoapi] /v1/chat/completions chain ok served-by=${result.__chainAttempted?.findLast?.(a => a.ok)?.model || 'unknown'} attempts=${result.__chainAttempted?.length || 1} cache=${cacheWrap.hit}`);
      }
