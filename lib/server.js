...
/**
 * Retry-After seconds advertised when every chain link failed. Deliberately
 * short: chain exhaustion here is usually a transient upstream wobble that the
 * next request already re-attempts (the chain re-probes per request), not a
 * stated provider deadline. Override with ACPTOAPI_CHAIN_EXHAUSTED_RETRY_AFTER_S.
 */
...
async function handleXaiOauthChat(req, res, body) {
  ...
  // xAI reserves max_tokens against the prompt budget -- see
  // shouldOmitMaxTokens. This path does not go through clampMaxTokensForModel.
  ...
  try {
    ...
    if (r.status === 401) {
      // Reactive refresh: the proactive skew missed an early expiry, or the
      // token was revoked upstream. Refresh once and retry a single time.
    ...
    if (stream && r.body) {
      ...
      while (true) {
        ...
        if (chunkBuf.length > 8192) chunkBuf = chunkBuf.slice(-2048); // bound the scan buffer
      ...
      } catch { /* non-JSON body, e.g. error passthrough */ }
    ...
    if (!res.headersSent) jsonErr(res, 500, { message: e.message, type: 'api_error', code: 'upstream_error', hint: 'request to xai-oauth failed before a response could be streamed; check xAI status' });
    else { try { res.end(); } catch { /* already closed */ } }
  }
}

// Key rotation was previously logged on only one of the three rotation paths,
// so a chain-routed request rotated silently while a direct one announced it.
...
async function executeBrandModel(brandName, opts) {
  ...
  // openai-oauth (ChatGPT/Codex): unlike xai-oauth, there is no raw
  // openai-compat passthrough to hand-roll here -- lib/sdk.js's buildParams
  // already routes this brand to the 'openai-oauth' provider (Responses API
  // translation, lib/providers/openai-oauth.js), and sdk.chat() already
  // knows how to buffer a provider's raw event stream into a final
  // OpenAI-chat-shaped JSON via formats/openai.js -- reuse that generic
  ...
  delete upstreamBody.stream;
  // chain-machine.js's callOpts now carries the resolved per-link `timeout`
  // (see its own comment) so executeXaiOauthModel can bound its raw fetch --
  // this brand's upstream body must not forward that internal field
  // verbatim to the real provider's JSON API.
...
// Extensive retry on transient network/5xx failures -- when xai-oauth is
// explicitly pinned (model: 'xai-oauth/...') it is the ONLY link in the
// chain, so a single flaky attempt exhausts the whole chain immediately with
// no fallback candidate to advance to. Retrying the same link several times
// with backoff before giving up gives a lenient posture on a genuinely slow
// or momentarily unreliable connection instead of surfacing a hard failure
// after one shot. Does NOT retry on 4xx (auth/bad-request) -- those are not
// transient and retrying them just wastes time before the real error surfaces.
// Delegates to xai-oauth.js's chatCompletion, the single shared
// credential-aware implementation also used by readiness.js's probeOne (see
// that module's comment for why the two paths must not diverge).
async function executeXaiOauthModel(opts) {
  ...
  delete upstreamBody.timeout;
  // Previously called with no options at all, so xai-oauth.js's doFetch
  // attached no AbortSignal to the raw fetch -- a stalled TCP/TLS connection
  // to api.x.ai could hang indefinitely with nothing to cancel it, unlike
  // readiness.js's probeOne (below, same module) which always bounds its own
  // probe to CONF.probeTimeoutMs(). Passing the link's own timeout here (the
  // same ceiling chain-machine.js's outer Promise.race already promises the
  // caller) makes the fetch itself abort at that ceiling instead of leaving
  // a zombie request running past it. See xai-oauth.js's
  // XAI_OAUTH_DEFAULT_TIMEOUT_MS for the fallback when no per-link timeout
  // is present.
  //
  // `timeoutMs ? {timeoutMs} : {}` was wrong: a single-link chain (xai-oauth
  // explicitly pinned, no fallback candidate) resolves link.timeout to 0 --
  // server.js's own linkTimeout logic deliberately means "no fixed ceiling"
  // for exactly this shape. 0 is falsy, so the ternary dropped it and passed
  // {} instead, and chatCompletion's `timeoutMs != null ? timeoutMs :
  // XAI_OAUTH_DEFAULT_TIMEOUT_MS` then saw an ABSENT key (not the intended
  // 0) and fell back to the 120s default -- silently reinstating the exact
  // fixed ceiling the single-link-chain fix existed to remove. Live-
  // witnessed: a pinned xai-oauth/grok-4.6 High-reasoning-effort turn hit
  // "every chain link failed: xai-oauth/grok-4.6 (timeout)" repeatedly
  // (three times in one session) with acptoapi and grok both independently
  // confirmed healthy moments later via a direct out-of-band call --
  // consistent with a legitimate long stream being cut off mid-flight by
  // this stale 120s ceiling, not a real provider failure. `!= null` (not a
  // truthiness check) is required to distinguish "0, no ceiling" from
...
async function handleChat(req, res, backends) {
  ...
  if (body.model && process.env.ACPTOAPI_DISABLE_CHAIN !== '1') {
    ...
    // 0) comma-separated chain string? 1) named chain? 2) directly-routable
    // single-link? 3) fall back to default curated queue
    let autoLinks = null;
    // splitBrandModel's regex (^([a-z0-9-]+)\/(.+)$) matches a raw comma
    // string like 'deepseek/bogus, xai-oauth/grok-4.6' as ONE link (prefix
    // captured, the rest -- comma and all -- swallowed into the "model"
    // capture group), so without this split the directlyRoutable check below
    // never even sees more than one link and the chain exhausts after a
    // single attempt. sdk.js's own chat()/chatChain() already splits via
    // parseCommaList before building links; this endpoint must match that.
    ...
    if (!autoLinks) {
      ...
      // Auto/unknown uses the LIVE builder so ACP_MODEL_CACHE refreshes before
      // chain build. A pinned directlyRoutable id skips that refresh:
      // buildAutoChainLive returns addLink(targetModel) only.
      const hasTools = Array.isArray(body.tools) && body.tools.length > 0;
      if (directlyRoutable) {
    ...
    try {
      ...
      // ACPTOAPI_LINK_TIMEOUT_MS -- and, since this is the ONLY var that
      // actually reaches runChat's opts.timeout for /v1/chat/completions
      // traffic (it always overrides chain-machine.js's own
      // ACPTOAPI_CHAIN_LINK_TIMEOUT_MS-derived default), it now falls back to
      // THAT var too before the hardcoded 120000 -- previously a caller who
      // set only ACPTOAPI_CHAIN_LINK_TIMEOUT_MS (a name that reads as "the"
      // chain link timeout) silently had zero effect on real chat-completions
      // traffic, witnessed live 2026-08-31: a hung request blocked the whole
      // chain, including streaming (zero bytes reached the client), for the
      // full stale 120000ms default despite that var being set to something
      // shorter. Lenient by default (internet + reasoning models are
      // genuinely slow) -- reasoning models (xai-oauth/grok-* et al) spend a
      // large chunk of the wall-clock on hidden reasoning tokens before any
      // visible content lands, and non-streamed requests only resolve once
      // the FULL body is back, so a large max_tokens (the norm for these
      // models) can legitimately take minutes. Witnessed: a 500-word-essay
      // request with max_tokens=500000 to xai-oauth/grok-4.6 hit a 25s link
      // timeout, chain-exhausted, and returned the canned "All upstream
      // providers are currently unavailable" fallback -- indistinguishable
      // from a real outage even though xAI was still actively generating.
      // Reasoning-capable brands get an even longer ceiling on top of the
      // already-lenient base default.
      ...
      // A single-link chain (a pinned model, e.g. auto-chain.js's
      // buildAutoChain: "a specific model is requested, use ONLY that
      // model") has nowhere to fall back TO -- the whole point of a link
      // timeout is to fast-fail a stalling provider so the chain can move to
      // the NEXT candidate; with no next candidate, cutting the request off
      // only turns real, still-in-progress work into a manufactured failure.
      // Witnessed live: a pinned xai-oauth/grok-4.6 request deep into a long
      // multi-step conversation (large accumulated context, genuinely slower
      // generation) hit the 300000ms reasoning ceiling and was reported as
      // "all upstream providers unavailable" while xAI was still actively
      // responding -- indistinguishable from a real outage, exactly the
      // failure mode this file's own comment above already names for a
      // shorter ceiling.
      //
      // ACPTOAPI_LINK_TIMEOUT_MS/ACPTOAPI_CHAIN_LINK_TIMEOUT_MS were
      // "explicit override always wins, even for single-link" by design --
      // but that design assumed a deployer setting them was making a
      // considered choice about single-link specifically. Live evidence
      // says otherwise: ~/.acptoapi/.env's own comment history for
      // ACPTOAPI_LINK_TIMEOUT_MS=60000 documents it was tuned entirely for
      // the MULTI-link auto-fallback case (opencode-zen free-tier models
      // with genuine ~50s response times, wanting to fail over to the next
      // candidate promptly) -- a value sized for "many fallback candidates,
      // fail fast and try the next one" was silently reapplied to "one
      // pinned model, zero fallback candidates, failing fast just means
      // failing", reintroducing the exact ceiling the single-link carve-out
      // exists to remove. A dedicated single-link-only override
      // (ACPTOAPI_SINGLE_LINK_LINK_TIMEOUT_MS) replaces the shared one for
      // a deployer who genuinely wants a real cap on single-link requests
      // specifically, rather than the multi-link-tuned var leaking in
      // unintentionally.
      ...
      // chain-machine.js's runStream/runChat separately enforce a WHOLE-CHAIN
      // wall-clock budget (CHAIN_WALL_CLOCK_BUDGET_MS, 90s default) that
      // gates whether a same-link retry is even attempted after a transient
      // failure (chainDeadline check) -- entirely independent of linkTimeout
      // above, and never overridden by any call site until now. Sized for
      // the multi-link case (give up fast, try the next provider); for a
      // single-link pinned chain there IS no next provider, so the same
      // "nowhere to fall back to" reasoning that zeroed linkTimeout applies
      // here too -- live-witnessed: a pinned xai-oauth/grok-4.6 stream ran
      // cleanly past linkTimeout's now-removed ceiling for 7+ minutes, then
      // hit one transient blip and failed immediately with "every chain
      // link failed (timeout)" because chainDeadline (fixed 90s from
      // request start) had long since passed, so the retry-eligible
      // failure was refused a retry instead of getting the lenient
      // same-link-retry posture this file's own runStream comment already
      // describes as the intended behavior for exactly this case.
      const chainBudgetMs = isSingleLinkChain
        ? Number(process.env.ACPTOAPI_SINGLE_LINK_CHAIN_BUDGET_MS) || 0
        : undefined;
      // Same shape and same reasoning as chainBudgetMs above, for
      // chain-machine.js's separately-gated SAME_LINK_RETRY_BUDGET_MS
      // (default sourced from ACPTOAPI_SAME_LINK_RETRY_BUDGET_MS) -- live-
      // witnessed set to 45000 (45s) in ~/.acptoapi/.env, its own comment
      // documenting that value was tuned for the multi-link case (many free
      // fallback candidates, fail a flaky lead fast). Unconditionally
      // applied, that same 45s window also refused a same-link retry for a
      // single-link pinned chain the moment any transient blip landed after
      // 45s of otherwise-healthy elapsed time -- exactly reproduced live: a
      // pinned xai-oauth/grok-4.6 stream's first blip at 60022ms elapsed hit
      // `elapsed >= 45000` and was refused a retry outright. 0 here means
      // Infinity to chain-machine.js's runStream/runChat (mirrors
      // chainBudgetMs's own 0-means-no-ceiling convention).
      const sameLinkRetryBudgetMs = isSingleLinkChain
        ...
        : undefined;
      // Real incremental streaming (2026-08-31): previously EVERY
      // /v1/chat/completions request, streamed or not, went through the
      // non-streamed runChat path below and awaited the FULL upstream
      // response before the streaming branch (further down this function)
      // synthesized a single fake SSE chunk from it -- witnessed live: a
      // client saw ZERO bytes for the entire ~45s a slow-but-working free
      // model took to respond, identical to no streaming at all. The real
      // machinery already existed and was already used elsewhere (lib/sdk.js
      // stream() -> translate() -> providers/openai.js streamOpenAI(), which
      // does genuine chunk-by-chunk SSE parsing against the real upstream) --
      // it just was never wired into this HTTP route. Only engaged when the
      // caller actually asked for streaming; a non-streaming request's
      // behavior (including its response-cache wrap, below) is completely
      // unchanged, zero regression risk there.
      if (wantsStream) {
        ...
        const attempted = [];
        // Headers are deferred until the served model is actually known
        // (set inside the streamFn callback below, which only fires once a
        // link is chosen) so streaming responses carry the same
        // X-Acptoapi-Served-Model/-Chain-Attempts diagnostic headers
        // non-streaming responses already have -- previously writeHead fired
        // immediately with none of that known yet, so streaming responses
        // never got them. ensureHeaders is idempotent and also covers the
        // exhaustion path (headers must go out exactly once regardless of
        // which branch reaches it first).
        const ensureHeaders = () => {
        ...
        try {
          ...
          for await (const ev of streamGen) {
            // Commit the status line only when this event actually produces
            // output. Calling ensureHeaders() for every event -- including
            // ones that write nothing -- sent a 200 before the chain had a
            // chance to fail, which then forced the exhaustion handler below
            // into its committed-status branch and produced an empty
            // 200 stream where a 503 was still possible.
            ...
            if (ev.type === 'text-delta' && ev.textDelta) {
              ...
              // providers/openai.js's streamOpenAI yields ONE consolidated
              // tool-call event per completed call (not incremental argument
              // deltas) when the caller-owns-execution shape (Array tools,
              // exactly what an HTTP /v1/chat/completions caller sends) is in
              // play -- see that file's own comment. Forwarded as a single
              // SSE delta carrying the full call, same as any OpenAI-compat
              // client already expects from a non-incremental tool-call chunk.
              sawAny = true;
          ...
          // Chain exhausted mid-stream. Which ending is available depends
          // entirely on whether the status line is still ours: nothing
          // written yet means a real error is possible, deltas already on the
          // wire means truncation is all that is left. Both branches below.
          ...
          // Nothing has reached the wire yet: the status line is still ours,
          // so fail properly instead of inventing an assistant turn. A 200
          // whose only content is the outage sentence is indistinguishable
          // from a real reply, and no OpenAI-compatible client retries a 200 --
          // agent runners recorded it as the model's answer and stalled
          // waiting for a human. See the non-streaming handler for the full
          // reasoning; both paths now answer the same way.
          ...
          // Deltas are already on the wire (or headers went out): the status
          // is committed, so the only honest ending is a clean truncation.
          // finish_reason is 'stop' for protocol compatibility, but the
          // exhaustion detail rides in the header set at ensureHeaders time.
          if (!headersSent) {
            headersSent = true;
            const extra = {};
      ...
      // Record the run in chain run-history so GET /v1/runs reflects the
      // /v1/chat/completions path too (previously only handleAnthropicMessages
      // recorded, so every chat-completions request showed attempted=null).
      // __chainAttempted carries the real per-link {model, ok, reason}; on a
      // cache hit there is no chain walk, so record a single served entry.
      try {
      ...
      if (wantsStream) {
        ...
        // Synthesize an SSE stream from the (non-streamed) chain result. This
        // MUST forward tool_calls, not just content -- a naive content-only
        // reconstruction silently drops any tool-call response, which looks
        // to the caller like the model produced empty text and stopped
        // (finish_reason was also hardcoded to 'stop' regardless of the real
        // reason, e.g. 'tool_calls' or 'length', masking why generation ended).
      ...
      // Nothing was streamed, so nothing is committed on the wire yet and the
      // status line is still ours to choose. Answer with a retryable error
      // rather than a fabricated assistant turn.
      //
      // The 200-with-outage-sentence shape this replaced was indistinguishable
      // from a real reply: an OpenAI-compatible client cannot retry a 200, so
      // agent runners recorded the sentence as the model's own answer, ended
      // the turn normally, and waited for a human to say "keep going" -- the
      // upstream failure was laundered into a successful-looking completion.
      // A 503 with Retry-After is the same information in a form every client
      // already knows how to act on, and the per-link detail still rides in
      // X-Acptoapi-Chain-Exhausted for anyone reading it.
      //
      // A partial stream keeps the old behavior (see the streaming catch
      // above, guarded by sawAny): once deltas are on the wire the status is
      // long since sent, and a clean truncation is the only option left.
      if (!res.headersSent) {
        try { res.setHeader('Retry-After', String(CHAIN_EXHAUSTED_RETRY_AFTER_S)); } catch {}
      }
...
async function handleAnthropicMessages(req, res, backends) {
  ...
  // Only require an auth header when the server actually has a gateway key
  // configured -- matches the general gateway auth (see requireAuth below) and
  // the /v1/chat/completions path, both of which serve without auth when no
  // ACPTOAPI_API_KEY/AGENTAPI_API_KEY is set. Previously this unconditionally
  // 401'd every /v1/messages request on a keyless server, breaking the
  // auth-optional SDK-drop-in contract (the path Claude Code's Anthropic SDK
  // calls).
...
function createServer({ port = 4800, backends = {}, queuesProvider = null } = {}) {
  ...
  return new Promise((resolve, reject) => {
    server.listen(port, host, () => {
      ...
      // Staggered at 5s/6s/8s/10s rather than all firing together: witnessed
      // live all three network-bound boot tasks (model-probe-live,
      // swe-bench-scores, brand-catalog) landing on the SAME 5000ms timer
      // simultaneously, competing for CPU/network alongside ACP daemon
      // autolaunch -- brand-catalog's refreshAll (individually ~1.7-5s per
      // brand when run alone) timed out on every single brand (nvidia,
      // opencode-zen, openrouter) under that contention, leaving
      // buildAutoChain stuck on stale/wrong static DEFAULT_MODELS ids for a
      // full TTL_MS window after every cold boot.
      ...
      // Same fire-and-forget boot pattern for the live SWE-bench Pro score
      // fetch (lib/swe-bench-scores.js): getModelScore() reads a sync disk
      // cache, so something must populate it before the ranking paths that
      // consume it (buildAutoChain's rankLinks tiebreak, getAvailableModels'
      // sort) run against real data instead of an empty/stale cache. Not
      // gated on ACPTOAPI_LIVE_PROBE (that flag is about provider
      // reachability probing, an unrelated concern) -- only
      // ACPTOAPI_DISABLE_BOOT_PROBE/ACPTOAPI_DISABLE_PROBE (the existing
      // "no network calls at boot" convention) skip it. refreshSweBenchScoresLive
      // itself no-ops if the on-disk cache is still within its own TTL, so a
      // fast restart never re-fetches needlessly.
      if (process.env.ACPTOAPI_DISABLE_BOOT_PROBE !== '1' &&
          process.env.ACPTOAPI_DISABLE_PROBE !== '1') {
      ...
      // Live brand-catalog refresh (lib/brand-catalog.js): buildAutoChain reads
      // brandCatalog.getCachedModels() synchronously (no network) to expand each
      // brand provider from its live model list instead of a single hardcoded
      // default. Something must populate that disk cache before the first
      // `auto`/chain request, or the very first chain falls back to the static
      // one-model-per-provider shape. refreshAll is TTL-gated and staggered
      // (bounded concurrency + jitter) so it never storms providers, and each
      // brand is a distinct host. Gated by the same DISABLE_BOOT_PROBE/
      // DISABLE_PROBE conventions; opt out of the catalog entirely with
      // ACPTOAPI_DISABLE_BRAND_CATALOG=1.
      if (process.env.ACPTOAPI_DISABLE_BOOT_PROBE !== '1' &&
          process.env.ACPTOAPI_DISABLE_PROBE !== '1' &&
      ...
      // Preemptive readiness prober (lib/readiness.js): keeps the top-K chain
      // candidates warm by real 1-token probes, feeding lib/availability.js so
      // the chain LEADS with a recently-verified live model and a user request's
      // fallback time approaches zero. Starts the recurring pass + fires one
      // warm-up pass ~8s after boot (after the brand-catalog refresh above has
      // populated the candidate set, since deriveCandidates reads buildAutoChain).
      // Budget-bounded and traffic-deferring (see readiness.js); opt out with
      // ACPTOAPI_DISABLE_READINESS=1. Gated by the same boot-probe conventions.
      if (process.env.ACPTOAPI_DISABLE_BOOT_PROBE !== '1' &&
          process.env.ACPTOAPI_DISABLE_PROBE !== '1' &&
