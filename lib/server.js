...
async function executeXaiOauthModel(opts) {
  ...
  let lastErr = null;
  for (let attempt = 0; attempt <= XAI_OAUTH_MAX_RETRIES; attempt++) {
    try {
      let r = await doFetch(creds.bearer);
      if (r.status === 401) {
        const refreshed = await xaiOauth.forceRefresh();
      ...
      if (!r.ok) {
        ...
        e.status = r.status;
        if (r.status === 429) e.code = 'RATE_LIMIT';
        // Retry on 429/5xx (transient/quota); 4xx other than 429 is not retried.
        if ((r.status === 429 || r.status >= 500) && attempt < XAI_OAUTH_MAX_RETRIES) {
          lastErr = e;
          console.log(`[acptoapi] xai-oauth: attempt ${attempt + 1}/${XAI_OAUTH_MAX_RETRIES + 1} failed (HTTP ${r.status}), retrying in ${XAI_OAUTH_RETRY_BASE_MS * Math.pow(2, attempt)}ms`);
          await new Promise(res => setTimeout(res, XAI_OAUTH_RETRY_BASE_MS * Math.pow(2, attempt)));
          continue;
        }
        throw e;
      }
      ...
      if (isTransientFetchError(e) && attempt < XAI_OAUTH_MAX_RETRIES) {
        lastErr = e;
        console.log(`[acptoapi] xai-oauth: attempt ${attempt + 1}/${XAI_OAUTH_MAX_RETRIES + 1} network error (${e.message}), retrying in ${XAI_OAUTH_RETRY_BASE_MS * Math.pow(2, attempt)}ms`);
        await new Promise(res => setTimeout(res, XAI_OAUTH_RETRY_BASE_MS * Math.pow(2, attempt)));
        continue;
      }
      throw e;
...
async function handleChat(req, res, backends) {
  ...
  if (body.model && process.env.ACPTOAPI_DISABLE_CHAIN !== '1') {
    ...
    try {
      ...
      // ACPTOAPI_LINK_TIMEOUT_MS. Lenient by default (internet + reasoning
      // models are genuinely slow) -- reasoning models (xai-oauth/grok-* et
      // al) spend a large chunk of the wall-clock on hidden reasoning tokens
      // before any visible content lands, and non-streamed requests only
      // resolve once the FULL body is back, so a large max_tokens (the norm
      // for these models) can legitimately take minutes. Witnessed: a
      // 500-word-essay request with max_tokens=500000 to xai-oauth/grok-4.6
      // hit a 25s link timeout, chain-exhausted, and returned the canned
      // "All upstream providers are currently unavailable" fallback --
      // indistinguishable from a real outage even though xAI was still
      // actively generating. Reasoning-capable brands get an even longer
      // ceiling on top of the already-lenient base default.
  ...
  return new Promise((resolve, reject) => {
    server.listen(port, host, () => {
      ...
      if (process.env.ACPTOAPI_DISABLE_BOOT_PROBE !== '1' &&
        ...
        readiness.start();
        const readinessWarmTimer = setTimeout(() => { readiness.runOnce().catch(() => {}); }, 8000);
