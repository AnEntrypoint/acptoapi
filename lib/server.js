...
async function executeXaiOauthModel(opts) {
  ...
  // is present.
  return xaiOauth.chatCompletion(upstreamBody, timeoutMs ? { timeoutMs } : {});
}
...
async function handleChat(req, res, backends) {
  ...
  if (body.model && process.env.ACPTOAPI_DISABLE_CHAIN !== '1') {
    ...
    try {
      ...
      // shorter ceiling. An explicit env override still wins if the
      // deployer wants a real cap even for a single-link chain.
      ...
      const linkTimeout = explicitLinkTimeout
        || (isSingleLinkChain ? 0 : (isReasoningModel ? Number(process.env.ACPTOAPI_REASONING_LINK_TIMEOUT_MS) || 300000 : 120000));
