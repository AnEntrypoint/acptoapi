    ...
    try {
      ...
      if (wantsStream) {
        ...
        try {
          ...
          // Chain exhausted mid-stream -- same "never surface a raw provider
          // error, answer with a clean assistant message" contract the
          // non-streaming catch block below already honors, just emitted as
          // SSE instead of a JSON body. Headers may or may not have gone out
          // yet (ensureHeaders is idempotent either way); the exhaustion
          // detail rides in X-Acptoapi-Chain-Exhausted the same way the
          // non-streaming path uses it, only settable here if this is the
          // first write of the response.
          ...
          const content = sawAny ? '' : 'All upstream providers are currently unavailable. Please retry shortly.';
          if (content) sse(res, makeChunk(rid, mdl, { content }));
