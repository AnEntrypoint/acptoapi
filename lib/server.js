...
async function handleAnthropicMessages(req, res, backends, claudeBin) {
  ...
  // Bare claude-* model names (no slash) from Claude Code CLI route to auto-chain
  // unless ANTHROPIC_API_KEY is set (in which case go direct to Anthropic).
  ...
  if (!body.model || body.model === 'auto') {
    const autoLinks = buildAutoChain(body.model === 'auto' ? undefined : body.model);
    if (autoLinks.length > 0) body.model = autoLinks[0].model;
  }
  if (isBareClaudeName && process.env.ANTHROPIC_API_KEY) {
    body.model = 'anthropic/' + body.model;
  }
  const inferredProvider = (() => {
    if (body.model && body.model.startsWith('claude/')) return 'claude';
    if (body.model && /^minimaxai\//i.test(body.model)) return 'nvidia';
    if (body.model && body.model.startsWith('nvidia/')) return 'nvidia';
    if (body.model && /^(z-ai\/|meta\/|qwen\/|deepseek\/|mistralai\/|microsoft\/|google\/)/i.test(body.model)) return 'openai-compat';
    if (body.model) { const sub = splitBrandModel(body.model); if (sub && isBrand(sub.prefix)) return 'brand'; }
    if (process.env.NVIDIA_API_KEY) return 'nvidia';
    return 'gemini';
  })();
  const provider = req.headers['x-provider'] || inferredProvider;
  let brandUrl, brandApiKey;
  if (provider === 'brand') {
    const sub = splitBrandModel(body.model);
    const brand = sub ? getBrand(sub.prefix) : null;
    if (!brand) return json(res, 400, { error: { type: 'invalid_request_error', message: `Unknown brand prefix in model: ${body.model}` } });
    const apiKeyVal = process.env[brand.envKey];
    if (!apiKeyVal) return json(res, 401, { error: { type: 'authentication_error', message: `Missing ${brand.envKey} for model: ${body.model}` } });
    brandUrl = brand.url;
    brandApiKey = apiKeyVal;
    body.model = sub.model;
  }
  // Strip vendor prefix for upstream calls (e.g. nvidia/deepseek-ai/... → deepseek-ai/...)
  if (provider === 'openai-compat' || provider === 'nvidia') {
    const sub = splitBrandModel(body.model);
    if (sub) body.model = sub.model;
  }
  dlog('anthropic.route', { provider, normalizedModel: body.model });
  logAnthropic({ action: 'route', provider, model: body.model, originalModel: body.originalModel || null, streaming: body.stream });
...
if (streaming) {
    ...
    try {
      const effectiveProvider = provider === 'brand' ? 'openai-compat' : provider;
      const baseOpts = { from: 'anthropic', to: 'anthropic', provider: effectiveProvider, ...body };
      const opts = effectiveProvider === 'openai-compat' || effectiveProvider === 'nvidia'
        ? {
            ...baseOpts,
            url: provider === 'brand' ? brandUrl : `${(process.env.OPENAI_API_BASE || 'https://integrate.api.nvidia.com/v1').replace(/\/$/, '')}/chat/completions`,
            apiKey: provider === 'brand' ? brandApiKey : (process.env.OPENAI_API_KEY || process.env.NVIDIA_API_KEY),
        ...
        : baseOpts;
      dlog('anthropic.stream.start', { provider, model: body.model, hasApiKey: !!(opts.apiKey || process.env.OPENAI_API_KEY || process.env.NVIDIA_API_KEY) });
      logAnthropic({ action: 'stream_start', provider, model: body.model, durationMs: Date.now() - startedAt });
      for await (const ev of translate(opts)) {
        if (ev.type === 'sse' && ev.raw) res.write(ev.raw + '\n');
      }
      dlog('anthropic.stream.done', { provider, model: body.model, durationMs: Date.now() - startedAt });
      logAnthropic({ action: 'stream_done', provider, model: body.model, durationMs: Date.now() - startedAt });
    } catch (e) {
      dlog('anthropic.stream.error', { provider, model: body.model, durationMs: Date.now() - startedAt, error: { message: e.message, stack: e.stack } });
      logAnthropic({ action: 'stream_error', provider, model: body.model, durationMs: Date.now() - startedAt, error: e.message });
    ...
    try {
      ...
      const opts = effectiveProvider === 'openai-compat' || effectiveProvider === 'nvidia'
        ...
        : baseOpts;
      const result = await buffer(opts);
      dlog('anthropic.out.raw', { durationMs: Date.now() - startedAt, result });
      if (!isValidAnthropicMessageResponse(result)) {
        dlog('anthropic.out.reject', { reason: 'malformed_response', durationMs: Date.now() - startedAt, result });
        logAnthropic({ action: 'reject_malformed', provider, model: body.model, durationMs: Date.now() - startedAt });
        return json(res, 502, { error: { type: 'api_error', message: 'Upstream returned malformed Anthropic response' } });
      }
      if (result.stop_reason === 'error') {
        dlog('anthropic.out.reject', { reason: 'error_stop_reason', durationMs: Date.now() - startedAt, result });
        logAnthropic({ action: 'reject_error_stop', provider, model: body.model, durationMs: Date.now() - startedAt });
        return json(res, 502, { error: { type: 'upstream_error', message: 'Upstream returned error stop_reason' } });
      }
      if (result.content.length === 0) {
        dlog('anthropic.out.reject', { reason: 'empty_content', durationMs: Date.now() - startedAt, result });
        logAnthropic({ action: 'reject_empty', provider, model: body.model, durationMs: Date.now() - startedAt });
        return json(res, 502, { error: { type: 'upstream_error', message: 'Upstream returned empty content' } });
      }
      dlog('anthropic.out.ok', { durationMs: Date.now() - startedAt, stop_reason: result.stop_reason, contentBlocks: result.content.length });
      logAnthropic({ action: 'success', provider, model: body.model, durationMs: Date.now() - startedAt, stop_reason: result.stop_reason });
      json(res, 200, result);
    } catch (e) {
      dlog('anthropic.out.error', { durationMs: Date.now() - startedAt, error: { message: e.message, stack: e.stack } });
      logAnthropic({ action: 'error', provider, model: body.model, durationMs: Date.now() - startedAt, error: e.message });
