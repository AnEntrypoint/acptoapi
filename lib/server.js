...
async function handleAnthropicMessages(req, res, backends, claudeBin) {
  ...
  const inferredProvider = (() => {
    ...
    if (body.model && /^(z-ai\/|meta\/|qwen\/|deepseek\/|mistralai\/|microsoft\/|google\/|nvidia\/)/i.test(body.model)) return 'openai-compat';
    return 'gemini';
  })();
    ...
    try {
      ...
      const opts = provider === 'openai-compat' || provider === 'nvidia'
        ? {
            ...
            apiKey: process.env.OPENAI_API_KEY || process.env.NVIDIA_KEY,
        ...
        : baseOpts;
      dlog('anthropic.stream.start', { provider, model: body.model, hasApiKey: !!(opts.apiKey || process.env.OPENAI_API_KEY || process.env.NVIDIA_KEY) });
