...
if (streaming) {
    ...
    try {
      const baseOpts = { from: 'anthropic', to: 'anthropic', provider, ...body };
      const opts = provider === 'openai-compat' || provider === 'nvidia'
        ? {
            ...baseOpts,
            url: `${(process.env.OPENAI_API_BASE || 'https://integrate.api.nvidia.com/v1').replace(/\/$/, '')}/chat/completions`,
