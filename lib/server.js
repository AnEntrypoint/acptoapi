...
async function listModels(queuesProvider) {
  ...
  const seen = new Set();
  const data = [];
  // Always include ACP catalog entries so chains can target them and trigger
  ...
  try {
    ...
    for (const m of cjModels) {
      ...
      if (seen.has(id)) continue;
      seen.add(id);
      data.push({ id, object: 'model', owned_by: 'chatjimmy', created });
    }
  } catch {}
...
async function executeAcpModel(prefix, opts, backends) {
  ...
  const msgPromise = sendMessage(backend, sessionId, prompt, sub).then(r => r.text()).catch(e => { throw e; });
  try {
...
async function executeViaProvider(name, opts) {
  ...
  if (name === 'chatjimmy' || name === 'openai-compat' || name === 'nvidia' || name === 'ollama' || name === 'anthropic' || name === 'gemini') {
...
async function executeForModel(opts, backends) {
  ...
  const e = new Error(`Unknown model '${opts.model}'`); e.status = 400; throw e;
}
...
async function handleChat(req, res, backends) {
  ...
  if (body.model && process.env.ACPTOAPI_DISABLE_CHAIN !== '1') {
    ...
    if (!autoLinks) {
      ...
      if (directlyRoutable) {
        autoLinks = buildAutoChain(body.model);
      } else {
        console.log(`[acptoapi] /v1/chat/completions unknown model '${body.model}', falling through to default queue`);
        autoLinks = buildAutoChain();
      }
    }
    try {
      const { runChat } = require('./chain-machine');
      const fallbackOn = ['error', 'rate_limit', 'timeout', 'empty', 'auth', 'fetch_failed'];
      const result = await runChat(
        autoLinks,
        { fallbackOn, _requestedModel: body.model, sampler },
        async (callOpts) => {
          try {
      ...
      console.log(`[acptoapi] /v1/chat/completions chain ok served-by=${result.__chainAttempted?.findLast?.(a => a.ok)?.model || 'unknown'} attempts=${result.__chainAttempted?.length || 1}`);
      ...
      console.error(`[acptoapi] chain exhausted: ${e.message}`);
      const attempted = e.attempted || [];
      if (wantsStream) {
        res.writeHead(502, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ error: { message: 'all chain links failed', attempted, last_error: e.message } }));
        return;
      }
      return json(res, 502, { error: { message: 'all chain links failed', attempted, last_error: e.message } });
    }
  }
...
function createServer({ port = 4800, backends = {}, queuesProvider = null } = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (url.pathname === '/v1/embeddings' && req.method === 'POST') return handleEmbeddingsGone(req, res);
  ...
  return new Promise((resolve, reject) => {
    server.listen(port, host, () => {
      ...
      // ACP backend auto-launch is OPT-IN only via ACPTOAPI_ENABLE_ACP_AUTOLAUNCH=1.
      // Default: skip startup-time launch. Eager-spawning all 11 ACP backends
      // causes a popup storm on Windows because each `bun x <package>` install
      // step (during cache-miss) bypasses CREATE_NO_WINDOW propagation. Lazy
      // spawn (on first chat to that backend) is implemented via the chat path's
      // resolveBackend → spawnDaemon call chain. Users who want all backends
      // pre-warmed at startup can set the env explicitly.
      if (process.env.ACPTOAPI_ENABLE_ACP_AUTOLAUNCH === '1') {
        try {
