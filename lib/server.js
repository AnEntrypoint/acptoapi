...
async function handleAnthropicMessages(req, res, backends, claudeBin) {
  ...
  let queueLinks;
  const wantLive = process.env.ACPTOAPI_LIVE_PROBE === '1' || req.headers['x-live-probe'] === '1';
  if (wantLive) {
    try {
      const { getOrRefresh, buildChainFromProbe } = require('./model-probe-live');
      const probed = await getOrRefresh({ log: m => console.log(m) });
      queueLinks = buildChainFromProbe(probed);
