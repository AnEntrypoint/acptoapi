...
function render() {
  const lines = [];
  lines.push('# HELP acptoapi_uptime_seconds Process uptime in seconds');
  lines.push('# TYPE acptoapi_uptime_seconds gauge');
  lines.push(`acptoapi_uptime_seconds ${(Date.now() - startTime) / 1000}`);
  const seenC = new Set();
...
module.exports = { inc, observe, render };
