...
// Some brands' catalog endpoint enumerates a WIDER model set than their chat
// endpoint actually serves -- the catalog answers "what exists on this key's
// account", not "what this specific chat URL will accept". Witnessed live
// (2026-07-22, real CODESTRAL_API_KEY): codestral's catalog is deliberately
// read from api.mistral.ai/v1/models (see the override above, since
// codestral.mistral.ai/v1/models itself 404s) and returns 60 models, but
// POSTing any non-codestral-branded id (mistral-medium-2505, devstral-latest,
// ...) to codestral.mistral.ai/v1/chat/completions returns 403
// tier_not_allowed or 400 invalid_model. Narrowing to the codestral-branded
// prefix still leaves codestral-embed/codestral-embed-2505 (embeddings-only,
// also 400 invalid_model on the chat endpoint) -- excluded explicitly since
// this catalog feeds chat-chain candidates, not an embeddings chain. Only
// codestral-2508/codestral-latest are actually chat-servable. Left
// unfiltered, the other 58 occupied real chain slots guaranteed to fail, and
// each failure cascaded sampler_backoff across the rest of the
// mistral/codestral pool. A `false`/absent entry means "no extra filtering;
// every enumerated id is trusted".
const MODELS_FILTER = {
  codestral: (id) => /^codestral/i.test(id) && !/embed/i.test(id),
  // openrouter's /v1/models enumerates ":batch"-suffixed ids that its
  // synchronous /chat/completions surface always 404s ("This model is only
  // available through the Batch API. Use the /api/beta/batches endpoint
  // instead.") -- witnessed live across every :batch id readiness probed.
  // Same over-enumeration shape as codestral above: the catalog answers
  // "what exists", not "what this endpoint will serve".
...
// NOTE on credit-dead (402/CreditsError): unlike auth-dead, this is
// deliberately NOT tracked brand-wide here. A multi-model aggregator brand
// (witnessed: opencode-zen) can have some models genuinely free/working
// (nemotron-3-ultra-free) and others permanently 402ing on the SAME account
// (claude-opus-5, grok-4.6) -- a brand-wide exclusion wrongly took the
// working free models out of the chain alongside the dead paid ones. Credit-
// dead tracking lives in lib/availability.js instead, keyed per full model
// id (markCreditDead/isCreditDead), fed by chain-machine.js's
// markProviderFailed on a live 402 response and consulted in preCheck and
// auto-chain.js's per-brand model ranking. The catalog probe below still
// records reason:'http_402' when a CreditsError body surfaces under the
// /models endpoint itself (rare -- most brands' catalog is billing-blind),
// purely as diagnostic info via peek()/reason() -- it does not gate anything
// on its own.
...
// A single retry on timeout: witnessed live (nvidia, a ~90-model catalog)
// that a transient network blip can trip the probe's own AbortController
// well under the real ~1.7s response time the same endpoint returns on a
// plain retry -- without this, one flaky boot-time probe silently starves
// buildAutoChain of nvidia's entire live catalog for a full TTL_MS window,
// falling back to stale/wrong static DEFAULT_MODELS ids that then 404 on
// every real request. A genuinely slow/overloaded provider still exhausts
// both attempts and reports 'timeout' honestly.
async function probeBrand(name, _isRetry = false) {
  ...
  try {
    ...
    // Some brands (witnessed: opencode-zen) return 200 with a full model list
    // from /models regardless of billing state -- the catalog endpoint
    // answers "what models exist", not "can this account actually call
    // them". A 402/CreditsError body embedded in a 200 response would sail
    // straight through _extractIds and re-populate the chain with models
    // guaranteed to 402 on every real chat request. Detect the CreditsError
    // shape even under a 200 status and treat it the same as an HTTP error.
...
async function refreshAll(opts = {}) {
  ...
  const names = Object.keys(BRANDS).filter((n) => {
    ...
    // OAuth-token brands (envKey: null) never had a keyring key to enumerate
    // with; skip them here, same as an unconfigured env-key brand.
    if (!brand.envKey) return false;
    if (!keyring.hasAnyKey(brand.envKey)) return false;
