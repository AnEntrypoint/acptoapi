...
const EMBEDDING_BRANDS = {
  openai:    { url: 'https://api.openai.com/v1/embeddings',                envKey: 'OPENAI_API_KEY',    defaultModel: 'text-embedding-3-small' },
  together:  { url: 'https://api.together.xyz/v1/embeddings',              envKey: 'TOGETHER_API_KEY',  defaultModel: 'togethercomputer/m2-bert-80M-8k-retrieval' },
  mistral:   { url: 'https://api.mistral.ai/v1/embeddings',                envKey: 'MISTRAL_API_KEY',   defaultModel: 'mistral-embed' },
  voyage:    { url: 'https://api.voyageai.com/v1/embeddings',              envKey: 'VOYAGE_API_KEY',    defaultModel: 'voyage-3' },
  deepseek:  { url: 'https://api.deepseek.com/embeddings',                 envKey: 'DEEPSEEK_API_KEY',  defaultModel: 'deepseek-embedding' },
  // Local brand — no API key required. Default model: nomic-embed-text (768-dim).
  // Resolved via OLLAMA_URL env (defaults to http://localhost:11434). Request/
  // response shape differs from the cloud brands (uses /api/embeddings, no Bearer
  // auth) — server.handleEmbeddings has a local-brand branch that handles the
  // translation. This is the fallback path when no cloud provider keys are set.
  ollama:    { url: () => `${process.env.OLLAMA_URL || 'http://localhost:11434'}/api/embeddings`, envKey: null, local: true, defaultModel: 'nomic-embed-text' },
};
...
function getEmbeddingBrand(prefix) {
  const b = EMBEDDING_BRANDS[prefix];
  if (!b) return undefined;
  if (typeof b.url === 'function') {
    const fn = b.url;
    const out = { ...b };
