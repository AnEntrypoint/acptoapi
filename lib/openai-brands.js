...
const BRANDS = {
  ...
  // OAuth device-code variant (ported from NousResearch/hermes-agent): bearer
  // token comes from lib/xai-oauth.js's on-disk token store, not an env-var
  // key, so envKey is null here -- handleBrandChat special-cases this brand
  // (see lib/server.js) instead of going through the normal keyring path.
  'xai-oauth':    { url: 'https://api.x.ai/v1/chat/completions',                                                                     envKey: null },
  // OpenAI/ChatGPT (Codex) OAuth: same envKey:null special-casing as
  // xai-oauth above, but the bearer/base_url resolution AND the wire
  // protocol itself are handled by lib/providers/openai-oauth.js (a real
  // provider registration, not an openai-compat passthrough) -- ChatGPT's
  // backend speaks the Responses API, not Chat Completions, so this brand
  // is intercepted in sdk.js's buildParams before the generic openai-compat
  // branch. The url here is unused by that path; kept for shape parity.
  'openai-oauth': { url: 'https://chatgpt.com/backend-api/codex/responses',                                                          envKey: null },
  ...
  'opencode-zen': { url: 'https://opencode.ai/zen/v1/chat/completions',                                                            envKey: 'OPENCODE_ZEN_API_KEY' },
  ...
  // GitHub Models: OpenAI-compat inference gated by a GitHub token. Chat lives
  // under /inference/chat/completions (witnessed 200 for openai/gpt-4.1-nano);
  // the model list is a catalog shape at /catalog/models (see brand-catalog.js
  // MODELS_URL_OVERRIDE). 37 models live as of 2026-07-19.
  'github-models':{ url: 'https://models.github.ai/inference/chat/completions',                                                     envKey: 'GITHUB_API_KEY' },
...
module.exports = { BRANDS, isBrand, getBrand, listBrands, registerBrand, unregisterBrand };
