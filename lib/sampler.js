...
function createSampler() {
  ...
  // GET /v1/sampler/status's diagnostic dump. e.ok is written once by
  // markFailed and never reset except by a later markOk (a real success),
  // so a provider that failed even once kept reporting ok:false here long
  // after its own backoff window (e.nextCheck) had cleared and peekStatus
  // -- the function real routing decisions actually read -- already
  // considered it available again. Live-witnessed: xai-oauth showed
  // ok:false, failCount:0, nextCheckIn:0 (a one-off timeout already past
  // its own 30s ceiling) while every real request through it succeeded --
  // not a routing bug, but a misleading diagnostic that reads as "still
  // unhealthy" well after full recovery. Mirrors peekStatus's own
  // inBackoff-gated read of e.ok.
  function getStatus() {
    const now = Date.now();
