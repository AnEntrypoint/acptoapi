...
// A DIFFERENT, distinct tool_choice-adjacent failure from the thinking-mode
// one above: some backend nodes behind an openai-compat aggregator are
// actually Anthropic-format Go servers that reject a bare-STRING tool_choice
// outright -- "invalid anthropic tool_choice: json: cannot unmarshal string
// into Go value of type anthropic.anthropicToolChoice". This is the SAME
// underlying bug an earlier session mistakenly "fixed" by universally
// converting every openai-compat tool_choice to an OBJECT shape (shipped as
// 1.0.186, reverted as 1.0.187 after it broke real OpenAI-format backends
// that correctly expect the bare string) -- the earlier fix was right about
// the SYMPTOM and WRONG about the SCOPE: a blanket conversion breaks every
// genuinely OpenAI-format backend, but the specific Go backend that needs an
// object DOES exist and DOES need one. Live-witnessed AGAIN this session,
// now proven backend-dependent rather than model-dependent: the exact same
// model id (extra-0/minimax-m3) returned clean OpenAI-format JSON on one
// direct request and this Anthropic-format 400 on another, seconds apart --
// the aggregator load-balances the SAME model id across a POOL of backend
// nodes, and only SOME nodes in that pool are the Go/Anthropic-format kind.
// A per-model skip-cache (like _thinkingModeModels above) would be WRONG
// here since it is not the model that is incompatible, it is whichever
// backend node answers THIS particular request -- so this is a same-request
// retry only, never learned/cached across calls.
function _isAnthropicToolChoiceShapeError(text) {
...
async function callOpenAI({ url, apiKey, headers, body }) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 400 && _isForcedToolChoice(body.tool_choice) && _isAnthropicToolChoiceShapeError(t)) {
      // Same-request-only retry, never cached -- see _isAnthropicToolChoiceShapeError
      // above for why this must not be learned per-model like the thinking-mode
      // branch is (the SAME model id can hit either a compliant or a Go/Anthropic-
      // format backend node on different requests through this one aggregator URL).
      // {type:'any'} is Anthropic's own native forced-tool-choice shape -- exactly
      // what this backend's own anthropicToolChoice unmarshal expects.
    ...
    // xAI reports "maximum prompt length is 500000" when a caller-supplied
    // max_tokens was reserved against the prompt budget. One retry with
    // max_tokens stripped -- same shape as the thinking-mode tool_choice
    // retry above. See lib/model-token-limits.js shouldOmitMaxTokens.
...
// `thinking` is an Anthropic-only request field (Freddie forwards it
// verbatim on every link regardless of which upstream wire format actually
// serves the request -- reasoningEffort:'high' becomes {thinking:{...}} on
// the ORIGINAL caller-facing request, never stripped when a named chain
// falls through to a plain OpenAI-compat brand). No OpenAI-compat brand
// (groq, cerebras, mistral, together, ...) recognizes it; live-witnessed on
// Groq: HTTP 400 "property 'thinking' is unsupported", read by
// chain-machine's classifyError as a generic 'error' since no regex covers
// this shape, making a perfectly healthy model look dead on every retry.
...
async function* streamOpenAI({ url, apiKey, headers, body, tools, onStepFinish, streamGuard }) {
  ...
  while (true) {
    ...
    // `tools` is ambiguous by design across two callers: a standalone SDK
    // consumer that wants THIS function to run the whole agent loop passes an
    // execute-map object ({name: {description, parameters, execute}}), while
    // a caller that owns its own agent loop (freddie's machine.js, per
    // AGENTS.md's "acptoapi is THE SDK, no reimplementing" architecture)
    // passes the bare OpenAI-shaped schema ARRAY it also sent in the request
    // body -- acptoapi/lib/sdk.js's buildParams forwards that same array as
    // this param for exactly that reason. `tools?.[tc.name]` on an ARRAY is
    // never a valid name lookup (arrays index by number), so every call
    // through this path previously fell to `toolDef ? null : 'Tool not
    // found'` and reported EVERY tool call as missing regardless of whether
    // the caller's own registry actually had it -- root-caused live
    // 2026-08-24 against a real freddie turn (bash/read/glob/... all
    // "Tool not found" despite being real, registered freddie tools).
    // An Array `tools` means "the caller owns execution" -- surface the raw
    // tool-call and end the step here so control returns to that caller,
    // instead of guessing at execution this function was never meant to do
    // for this caller shape.
...
async function generateOpenAI({ url, apiKey, headers, body, tools }) {
  ...
  while (true) {
    ...
    // Same Array-vs-execute-map ambiguity as streamOpenAI above -- an Array
    // `tools` means the caller owns execution, so return the tool calls to
    // it instead of misreporting every one as "not found". Shape matches
    // streamOpenAI's own tool-call event vocabulary ({toolCallId, toolName,
    // args} with args already JSON.parse'd), not the raw OpenAI wire shape
    // (function.name/function.arguments-as-string) -- a caller handling both
    // entrypoints under the same "array tools" contract must not need two
    // different unwrapping paths for what is semantically the same result.
    if (Array.isArray(tools)) {
