...
function subscribeSSE(url, onEvent, onError) {
  ...
  (async () => {
    ...
    } catch (e) { if (e.name !== 'AbortError') { if (onError) onError(e); } }
  })();
  return () => ctrl.abort();
}
...
async function* streamACP({ url, model, messages, onStepFinish }) {
  ...
  const queue = []; let resolveNext = null; let done = false; let sseError = null;
  ...
  }, (e) => { sseError = e; done = true; if (resolveNext) { const r = resolveNext; resolveNext = null; r(); } });
