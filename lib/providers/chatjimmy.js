...
let _modelsCacheTtlMs = 5 * 60 * 1000;
const CHATJIMMY_NEGATIVE_TTL_MS = Number(process.env.ACPTOAPI_CHATJIMMY_NEGATIVE_TTL_MS) || 30000;
function rememberChatJimmyModels(ids, ttlMs) {
  _modelsCache = ids;
  _modelsCacheTs = Date.now();
  _modelsCacheTtlMs = ttlMs;
  return ids;
}
async function listChatJimmyModels() {
  const now = Date.now();
  if (_modelsCache && (now - _modelsCacheTs) < _modelsCacheTtlMs) return _modelsCache;
  try {
    const r = await fetch(`${BASE}/api/models`, { signal: AbortSignal.timeout(2000) });
    if (!r.ok) return rememberChatJimmyModels([DEFAULT_MODEL], CHATJIMMY_NEGATIVE_TTL_MS);
    const j = await r.json();
    const ids = (j.data || []).map(m => m.id).filter(Boolean);
    if (ids.length) return rememberChatJimmyModels(ids, 5 * 60 * 1000);
  } catch {}
  return rememberChatJimmyModels([DEFAULT_MODEL], CHATJIMMY_NEGATIVE_TTL_MS);
}
