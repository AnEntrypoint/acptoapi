...
// Runtime-injected keys (from file-based providers, not from env vars).
// Map<envKey, string[]>. Injected keys are checked AFTER env-var-sourced keys
// in _collectFromEnv, so explicit env vars always take precedence.
const _runtimeKeys = new Map();

function registerKey(envKey, value) {
  if (!envKey || !value) return;
  const trimmed = String(value).trim();
  if (!trimmed) return;
  if (!_runtimeKeys.has(envKey)) _runtimeKeys.set(envKey, []);
  const arr = _runtimeKeys.get(envKey);
...
// in PROVIDER_ORDER. Aliasing once at THIS chokepoint (every other function
// in this file funnels through _collectFromEnv) fixes every call site at
// once, rather than needing five separate hardcoded-name fixes that would
// inevitably drift out of sync again.
const _ENV_ALIASES = { GEMINI_API_KEY: ['GOOGLE_API_KEY'] };
...
function _collectFromEnv(envKey) {
  ...
  // Runtime-injected keys (from file-based extra providers) come last so env
  // vars always win.
  if (_runtimeKeys.has(envKey)) {
    for (const k of _runtimeKeys.get(envKey)) push(k);
  }
  // Same-credential aliases (see _ENV_ALIASES doc above) -- checked last so
  // the canonical name always wins when both happen to be set.
  if (_ENV_ALIASES[envKey]) {
    for (const alias of _ENV_ALIASES[envKey]) push(process.env[alias]);
  }
  return out;
}
...
function markKeyOk(envKey, key) {
  ...
  e.nextCheck = 0;
  e.lastReason = null;
  // Clear the failure timestamp on recovery too (mirrors lastReason); a healthy
  // key must not report a stale lastFailedAt to peekStatus.
  e.lastFailedAt = null;
}
