...
const FAILURE_TTL_MS = Number(process.env.ACPTOAPI_AVAILABILITY_FAILURE_TTL_MS) || 3600000; // 1h default - failures older than this are decayed so transient issues don't permanently demote models
// A model with at least this many samples and ZERO successes ever (lastSuccessTs
// still null) is exempt from FAILURE_TTL_MS decay -- see effectiveFailStreak's
// own comment for the live-witnessed bug this closes. Deliberately higher than
// MIN_SAMPLES_FOR_RANK: a genuinely new/rarely-tried model with only 1-2 samples
// still deserves the normal decay/second-chance treatment; this exemption is
// only for a model with an established, consistent, zero-success track record.
const MIN_FAILSTREAK_FOR_NEVER_SUCCEEDED = Number(process.env.ACPTOAPI_AVAILABILITY_NEVER_SUCCEEDED_THRESHOLD) || 5;
...
function createAvailabilityTracker({ persist = false } = {}) {
  ...
  //
  // Failure decay: failures older than FAILURE_TTL_MS are soft-decayed so a
  // transient rate-limit from hours ago doesn't permanently demote a model.
  // When the last failure is older than the TTL, the failStreak is treated
  // as if it were 0 for penalty purposes (the streak is NOT reset in cache so
  // a subsequent failure still compounds, but stale ones don't penalize).
  //
  // NEVER-SUCCEEDED EXEMPTION, live-witnessed: a model that has failed EVERY
  // single attempt it has ever made (lastSuccessTs still null after many
  // samples -- e.g. a local ollama model with nothing listening on that port,
  // or a retired/renamed model id) is not "a normally-healthy model having a
  // rough patch" -- decaying its failure history back to a neutral score
  // after FAILURE_TTL_MS just means the SAME broken model cycles back into
  // rotation forever, once per TTL window, regardless of how much real time
  // passes, because nothing about elapsed time makes a dead local port start
  // listening. Concretely: casey's own live isReachable() probe (deliberately
  // capped to the first few chain links to bound an uncancellable background
  // walk -- see freddie's acptoapi-bridge.js PROBE_CHAIN_LINK_CAP) hit
  // ollama/llama3.2 (48/48 failures, zero successes, last failure 98 minutes
  // -- past the 1h TTL -- before this exact check) as one of its first
  // candidates and got a false "unreachable" verdict, queuing a real
  // contact's message with no reply even though the real chain had plenty of
  // genuinely healthy models further down. A model earns exemption from this
  // rule the moment it succeeds even once (lastSuccessTs set) -- this is not
  // a permanent ban, only a refusal to let pure elapsed time manufacture a
  // clean bill of health for something that has never actually worked.
  function effectiveFailStreak(e) {
    if (!e || e.failStreak === 0) return 0;
    if (!e.lastSuccessTs && e.totalSamples >= MIN_FAILSTREAK_FOR_NEVER_SUCCEEDED) return e.failStreak;
    if (e.lastFailTs && (Date.now() - e.lastFailTs) > FAILURE_TTL_MS) return 0;
    return e.failStreak;
  }
  function score(model) {
    const e = cache.get(model);
    if (!e || e.totalSamples === 0) return 0;
    const fs = effectiveFailStreak(e);
    const failPenalty = Math.min(fs, 10) * 2;
