...
function buildAutoChain(targetModel) {
  ...
  if (targetModel && targetModel !== 'auto') {
    const link = { model: targetModel, fallbackOn: FALLBACK_ON };
    const score = getModelScore(targetModel);
  ...
  // Sort by SWE-bench score descending (highest first), unknowns at end
  sortByBenchmark(unsorted);
  links.push(...unsorted);
  return links;
}
