...
const DEFAULT_ORDER = ['anthropic','openrouter','groq','nvidia','cerebras','sambanova','mistral','codestral','qwen','zai','cloudflare','gemini','opencode-zen','ollama','kilo','opencode','qwen-code','codex-cli','copilot-cli','cline','hermes-agent','cursor-acp','codeium-cli','acp-cli'];
...
function hasProvider(name) {
  if (name in BUILTIN_KEYS) {
    ...
    if (['ollama', 'kilo', 'opencode', 'qwen-code', 'codex-cli', 'copilot-cli', 'cline', 'hermes-agent', 'cursor-acp', 'codeium-cli', 'acp-cli'].includes(name)) return true;
    return false;
  }
  if (isBrand(name)) {
