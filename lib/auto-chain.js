...
const DEFAULT_ORDER = ['anthropic','openrouter','groq','nvidia','cerebras','sambanova','mistral','codestral','qwen','zai','cloudflare','gemini','bedrock','opencode-zen','opencode-north','opencode','mimo','ollama','kilo','qwen-code','codex-cli','copilot-cli','cline','hermes-agent','cursor-acp','codeium-cli','acp-cli','chatjimmy'];
...
function buildAutoChain(targetModel, opts = {}) {
  ...
  // Two-tier sort: (1) DIRECT providers (sub-1s + reliable tool-calling) first,
  // then (2) ACP-wrapper + openrouter brand-routing tier (slow, often unauth).
  // Within each tier, sort by SWE-bench score descending. Then cap to ACP_AUTO_CHAIN_CAP
  // (default 12) so a single `auto` request can't walk 100+ dead-link
  // fallbacks for minutes  - first-paint UX requires sub-5s response.
  ...
  const direct = sortByBenchmark(pool.filter(l => !isAcpTier(l)));
  const wrapped = sortByBenchmark(pool.filter(l => isAcpTier(l)));
  const cap = Number(process.env.ACPTOAPI_AUTO_CHAIN_CAP) || 12;
  let sorted = [...direct, ...wrapped];

  // Tool-capability tier: when the caller advertised tools[], reorder the
  // chain so preferred-tools models lead, unknown follow, blocked sink last.
  // Within each capability bucket the prior direct/wrapped+benchmark order is
  // preserved (sort is stable).
  if (hasTools) {
    const capRank = (link) => {
      const t = modelCapabilityTools(link.model);
      if (t === true) return 0;
      if (t === null) return 1;
      return 2;
    };
    sorted = sorted.map((l, i) => ({ l, i, r: capRank(l) }))
      .sort((a, b) => (a.r - b.r) || (a.i - b.i))
      .map(x => x.l);
  }

  // Bump live-responding models ahead of recently-failing ones within the
  // existing tier/benchmark order. Neutral (no-data) models are left in place
  // via availability.rerank's stable sort, so this only kicks in once a model
  // has actually been observed by a real chain run (see lib/availability.js).
  if (process.env.ACPTOAPI_DISABLE_AVAILABILITY_RANK !== '1') {
    try { sorted = require('./availability').rerank(sorted); } catch {}
  }
...
module.exports = { buildAutoChain, buildAutoChainLive, refreshAcpModels, probeAcpModels, getDaemonFilter, filterDaemonModels, DEFAULT_ORDER, DEFAULT_MODELS, hasProvider, getOrder, getDefaultModel, getDefaultModelSync, ACP_MODEL_CACHE, modelCapabilityTools };
