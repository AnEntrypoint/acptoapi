...
function buildAutoChain(targetModel, opts = {}) {
  ...
  const live = available.filter(n => {
    if (ACP_DAEMONS.includes(n)) return true; // ACP daemons checked live via getLiveDaemons
  ...
  // registered endpoints from ~/.acptoapi/extra-providers.txt. They sit after
  // all built-in providers so they never displace known-working links, but are
  // still reachable when everything else fails.
  try {
