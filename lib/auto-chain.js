...
function xaiOauthLoggedIn() {
  try { return require('./xai-oauth').hasCredentials(); } catch { return false; }
}
...
function buildAutoChain(targetModel, opts = {}) {
  ...
  for (const name of live) {
    ...
    if (discovered.length > 0) {
      ...
      // The brand's fresh catalog probe returned 401/403 -- its configured key
      // is dead, so its static default would be guaranteed to 401 too. Exclude
      // it from the chain entirely rather than wasting a fallback slot on a
      // call that cannot succeed. (Witnessed: qwen with an invalid QWEN_API_KEY.)
      continue;
    } else {
