...
// STRUCTURALLY incompatible with a chat-completion request: not a "weak at
// tool-calling" chat model (soft-deprioritized below), but a model that
// literally does not expose a chat-completion endpoint shape at all --
// embeddings, TTS/speech synthesis, moderation/guard classifiers. A request
// against one of these can never succeed regardless of ranking, so these are
// HARD-EXCLUDED from the pool entirely (see hasTools filtering in
// buildAutoChain) rather than merely ranked last, where a small candidate
// pool could still spend one of the chain's limited slots on a guaranteed
// failure. orpheus/kokoro/parler-tts/xtts are named TTS model families whose
// ids don't literally contain "tts"/"whisper"/"speech" (Canopy Labs' Orpheus,
// Kokoro-TTS, Parler-TTS, Coqui XTTS) -- live-witnessed:
// groq/canopylabs/orpheus-arabic-saudi entered a real chat chain's candidate
// pool and was tried as a chat-completion model, wasting a full retry slot on
// a request that could never succeed. Same class of gap for embeddings:
// bge-m3/bge-large/gte/e5/nomic-embed/jina-embeddings are well-known
// text-embedding model families whose ids don't contain the literal
// substring "embed" either -- live-witnessed: nvidia/baai/bge-m3 (BAAI's
// widely-used BGE-M3 embedding model, zero chat-completion capability)
// entered a real chain and was tried as a chat model. Also structurally
// impossible: chart/plot-to-text and image-generation/diffusion models --
// live-witnessed nvidia/google/deplot (a chart-to-text VISION model, no
// text-chat-completion shape) and nvidia/google/diffusiongemma-26b-a4b-it
// (an image DIFFUSION model) both entered a real chain build alongside the
// embedding/TTS misses above.
const TOOLS_IMPOSSIBLE_RE = /(?:embed|embedqa|bge-(?:m3|large|base|small)|gte-(?:large|base|small)|e5-(?:large|base|small)|nomic-embed|jina-embeddings|nemoretriever|nemoguard|nemotron-?safety|safety-?guard|content-?safety|topic-?control|tts|whisper|speech|orpheus|kokoro|parler-?tts|xtts|guard|deplot|diffusion)/i;
// Genuinely CAN complete a chat request, but weak/unreliable at structured
// tool-calling specifically -- worth trying last, not excluding outright (a
// small/code-completion model still sometimes gets tool_choice right, and
// excluding it would shrink an already-thin chain for no structural reason).
// mistral-code-*/devstral-*/codestral-* (Mistral's own dedicated code-
// completion/agentic-coding model family, distinct from its general chat
// models like mistral-large/medium already in TOOLS_PREFERRED_RE) matches
// the same "purpose-built for code, not general tool-calling chat" class as
// code-llama/deepseek-coder/starcoder already here. Live-witnessed: FIVE of
// these (mistral-code-latest, mistral-code-fim-latest, devstral-2512,
// devstral-medium-latest, devstral-latest, mistral-code-agent-latest)
// occupied slots 2-7 of a single 12-slot chain build in a row (all from the
// same provider prefix's own catalog expansion), consuming half the chain's
// candidate budget on a real live turn before it ever reached a genuinely
// suitable model.
...
function modelCapabilityImpossible(modelId) {
  return !!(modelId && typeof modelId === 'string' && TOOLS_IMPOSSIBLE_RE.test(modelId));
}
function modelCapabilityTools(modelId) {
  if (!modelId || typeof modelId !== 'string') return null;
  if (TOOLS_IMPOSSIBLE_RE.test(modelId) || TOOLS_UNRELIABLE_RE.test(modelId)) return false;
...
function buildAutoChain(targetModel, opts = {}) {
  ...
  const addLink = (model) => {
    ...
    // Hard-exclude a STRUCTURALLY tool-incompatible model (TTS/embedding/
    // guard classifier) from a tool-required request entirely -- these can
    // NEVER succeed regardless of ranking, so a small candidate pool must
    // not spend one of its limited chain slots on a guaranteed failure (see
    // TOOLS_IMPOSSIBLE_RE's own doc above for the live-witnessed case). A
    // model that's merely UNRELIABLE at tool-calling (TOOLS_UNRELIABLE_RE)
    // still enters the pool -- it's soft-deprioritized by rankLinks below,
    // never excluded, since it genuinely can attempt the request.
    if (hasTools && modelCapabilityImpossible(model)) return;
    seenModels.add(model);
    const link = { model, fallbackOn: FALLBACK_ON, rrOrder: _rr++ };
  ...
  // then rotate one model per brand per pass.
  ...
  for (let depth = 0; depth < maxDepth; depth++) {
    for (const bucket of brandBuckets) {
      if (depth < bucket.length) addLink(bucket[depth]);
    }
  }
