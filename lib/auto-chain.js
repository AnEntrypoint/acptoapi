...
const DEFAULT_ORDER = ['anthropic','openrouter','groq','nvidia','cerebras','sambanova','mistral','codestral','qwen','zai','cloudflare','gemini','opencode-zen','ollama','kilo','opencode','gemini-cli','qwen-code','codex-cli','claude'];
...
function hasProvider(name) {
  if (name in BUILTIN_KEYS) {
    ...
    if (['ollama', 'kilo', 'opencode', 'gemini-cli', 'qwen-code', 'codex-cli', 'claude'].includes(name)) return true;
    return false;
  }
  if (isBrand(name)) {
