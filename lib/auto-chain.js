...
function buildAutoChain(targetModel, opts = {}) {
  ...
  const rankLinks = (links) => {
    ...
    // Final tiebreak is a random draw, not the original array index. On a cold
    // cache (no availability signal yet, av/cap/sc all tied at 0), sorting by
    // index would silently resolve to whatever produced `links`' original
    // order (DEFAULT_ORDER / PROVIDER_ORDER enumeration) -- a hardcoded
    // preference in disguise, exactly what this ranking exists to avoid.
    // Math.random() is intentional here (not the module-wide "no Date.now/
    // Math.random" ban that applies to gm workflow scripts) -- this is normal
    // runtime code, re-evaluated fresh on every buildAutoChain() call, so a
    // never-observed model gets a genuinely different tied position each
    // call rather than always losing to whichever provider happened to be
    // enumerated first.
    return links
      .map((l, i) => ({
        l, i,
      ...
      .sort((a, b) => (b.av - a.av) || (a.cap - b.cap) || (b.sc - a.sc) || (a.rnd - b.rnd))
      .map(x => x.l);
  };
