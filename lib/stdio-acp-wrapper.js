  ...
  async start() {
    return new Promise((resolve, reject) => {
      try {
        ...
        // a visible console. DO NOT OR with DETACHED_PROCESS (0x08): the
        // two flags are mutually conflicting per CreateProcess docs, and
        // when a console-subsystem descendant (node.exe inside bun.cmd →
        // kilo.cmd → node.exe chain) starts under DETACHED_PROCESS it
        // has no inherited console and Windows ALLOCATES A NEW ONE for
        // it on first stdout write — which is the popup we keep seeing.
        // CREATE_NO_WINDOW alone gives every descendant an invisible
        // console; nothing pops up. Node's detached:false keeps stdio
        // pipes connected (required for JSON-RPC).
        ...
        this.subprocess.on('exit', code => {
            this.emit('subprocess-exit', code);
            this.ready = false;
            // Release the HTTP listener so the next spawnDaemon attempt can rebind
            // the port. Without this, a failed-but-port-bound wrapper leaks the
            // port forever and every subsequent spawn fails with EADDRINUSE.
