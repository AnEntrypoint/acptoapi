      ...
      try {
        ...
        if (process.platform === 'win32') {
          ...
          if (/\.exe$/i.test(resolved)) {
            // Clean .exe path — spawn directly, no shell. CREATE_NO_WINDOW
            // propagates because the .exe is the immediate child.
            this.subprocess = spawn(resolved, this.args, spawnOpts);
          } else {
            // Shim or bare script — cmd.exe fallback. Outer cmd is
            // suppressed; downstream may still flash but at least the
            // immediate parent is hidden.
            const quoted = [resolved, ...this.args].map(a =>
