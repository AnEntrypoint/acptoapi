...
class StdioAcpWrapper extends EventEmitter {
  ...
  async start() {
    return new Promise((resolve, reject) => {
      try {
        // Windows console-window suppression: invoke cmd.exe /D /S /C
        // ourselves (instead of shell:true) so we control quoting and can
        // pass windowsVerbatimArguments:true. windowsHide:true sets
        // CREATE_NO_WINDOW, which propagates to the .cmd shim and the
        // node.exe it eventually exec's, so there is no console flash on
        // npx/bun chains. detached:true + proc.unref() on Windows ensures
        // the subprocess is spawned in its own process group.
        const spawnOpts = {
        ...
        if (process.platform === 'win32') {
          ...
          this.subprocess = spawn('cmd.exe', [`/D /S /C "${quoted}"`], {
            ...spawnOpts,
