...
class StdioAcpWrapper extends EventEmitter {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.subprocess.on('exit', code => {
            ...
            this.ready = false;
            this.releaseServerPortForNextSpawnAttempt();
        });
  ...
  releaseServerPortForNextSpawnAttempt() {
  ...
  async handleListModels(req, res) {
    try {
      ...
      const topLevelAvailableModels = result?.models?.availableModels;
      if (Array.isArray(topLevelAvailableModels) && topLevelAvailableModels.length) {
        models = topLevelAvailableModels.map(m => ({ id: m.modelId, name: m.name || m.modelId, description: m.description || null }));
