...
function set(key, value) {
  ...
  // Only cache JSON-serializable values. A value that cannot round-trip through
  // JSON could not be safely cloned on hit, and the old clone() silently returned
  // the ORIGINAL (mutable) reference in that case -- breaking the "callers can't
  // mutate the cached object" contract and corrupting the entry for everyone.
  // Rejecting at set() makes that corruption structurally impossible.
...
function clone(value) {
  // Safe by construction: set() only admits JSON-serializable values, so this
  // round-trip cannot throw. A clone is always returned -- never the original.
