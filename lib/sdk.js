...
// REVERTED (see git history for the removed attempt): a prior version of this
// file force-converted a bare-string tool_choice ('required'/'auto'/'none')
// into an object {type:'required'} for every openai-compat request, based on
// the WRONG assumption that {type:'required'} is a universally-safe superset.
// It is not -- OpenAI's real spec only accepts tool_choice as the bare string
// "none"/"auto"/"required", or an object shaped {type:"function",
// function:{name:"..."}} for ONE SPECIFIC named tool. There is no valid
// {"type":"required"} object form. Live-reproduced immediately in production
// after that change shipped: real OpenAI-format backends that previously
// worked started failing with `"tool_choice.type" must be "function", got
// "required"`. acptoapi's OWN reverse mapping already proves this --
// anthropic_tool_choice_to_openai (lib/formats/anthropic.js) converts
// Anthropic's {type:'any'} to the OpenAI bare STRING 'required', never to an
// object -- that evidence was available and should have been checked before
// the object-form change shipped, not after. The original Anthropic-format-
// aggregator bug (a bare string rejected by a Go anthropic.anthropicToolChoice
// unmarshal) is real, but per-model/per-backend detection or a provider-
// specific transform (this codebase's OWN tooluse transform in
// transformers.js, applied ONLY where a provider's config opts into it) is
// the correct fix -- never a blanket format change applied to every
// openai-compat request regardless of what the specific backend actually
// wants.
// xai-oauth (lib/xai-oauth.js) has envKey:null in openai-brands.js's BRANDS
// table by design -- there is no static API key for a device-code OAuth
// identity, so the r.env-keyed keyring.getKey() lookup above is always a
// no-op for this one prefix. Its real credential resolution (proactive
// refresh-before-expiry, reactive retry-once on 401) was previously wired
// ONLY into lib/server.js's handleXaiOauthChat -- the HTTP passthrough
// handler acptoapi's own standalone :4800 daemon uses. A consumer that
// calls sdk.chat()/chatChain() in-process (no HTTP server running, e.g.
// freddie's callLLM/readiness prober) went through buildParams with
// apiKey always undefined, sending the real x.ai request with no bearer
// token at all -- surfacing as "Incorrect API key provided" regardless of
// whether the stored token was fresh or expired, since the credential
// never reached the outbound request on this code path.
async function buildParams({ model, messages, system, tools, temperature, max_tokens, ...rest }) {
  ...
  if (r.prefix === 'xai-oauth') {
    ...
    // baseUrl is a bare API root (e.g. https://api.x.ai/v1, per xai-oauth.json's
    // stored base_url) -- server.js's handleXaiOauthChat appends /chat/completions
    // itself (server.js:434,694); mirror that exactly, a bare base_url alone
    // 404s (witnessed: "The requested resource was not found").
    const url = `${baseUrl.replace(/\/+$/, '')}/chat/completions`;
    return { provider: 'openai-compat', params: { url, apiKey: bearer, body: { model: r.model, messages, tools, temperature, max_tokens, ...clean }, tools } };
  }
  // openai-oauth (lib/openai-oauth.js): same envKey:null shape as xai-oauth
  // above, but ChatGPT's backend speaks the Responses API, not Chat
  // Completions, so there is no openai-compat URL/apiKey pair to build here
  // -- credential resolution (proactive refresh, reactive 401 retry) and
  // the actual request/response translation both live inside the
  // 'openai-oauth' provider (lib/providers/openai-oauth.js), which this
  // just routes to.
