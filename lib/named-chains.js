...
// Kept in sync with server.js's handleChat default fallbackOn -- a named
// chain is just a curated link list fed through the same chain-machine, so a
// narrower reason set here silently strands a chain on exactly the failure
// modes an OAuth-gated or rate-sensitive first link is most likely to hit
// (live-witnessed: xai-oauth's own 403 classifies as 'auth', which this list
// omitted, so a chain pinning it first never advanced to its fallback link
// on the one failure mode it exists to route around).
...
const BUILTIN = {
    // Free/cheap models first, then ACP daemons as fallback.
    ...
    // Free online-API models first (best-first per remoteopenclaw.com best-free-models-for-hermes:
    // Groq fastest + highest daily allowance, then OpenRouter free meta, then Google AI Studio),
    // then free ACP daemons as fallback.
    ...
    // Ordered free-for-Hermes chain: free online APIs ranked best-first, then no-key ACP daemons.
    ...
    // Live-verified 2026-07-30 (GET /v1/models against both accounts): opencode-zen's
    // hy3-free and cerebras's zai-org/glm-5.2 do not exist on these accounts (404/402).
    // Replaced with real, currently-servable models from the same two providers,
    // preferred/second ordering preserved.
