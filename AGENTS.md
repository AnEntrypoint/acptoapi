...
**xAI subtracts `max_tokens` from remaining prompt budget.** A Freddie-sized output cap (256000 default, or a 500000-shaped request) made grok-4.6 reject a prompt that still fit the real window: `invalid-argument` / "This model's maximum prompt length is 500000 but the request contains 500943 tokens", classified as a generic chain `error` and exhausting the only configured link. `clampMaxTokensForModel` returns `undefined` for the `xai-oauth` and `xai` prefixes so callers omit the field; `chatCompletion`, `handleXaiOauthChat`, `handleBrandChat`/`executeBrandModel` for `xai`, and `sdk.buildParams` drop it before the first upstream call. The openai-compat stream path retries once without `max_tokens` if that 400 still arrives. Do not re-send a DeepSeek/Gemini-sized `max_tokens` to api.x.ai.
...
`waitForLeadLinkPrecheck` runs one `preCheck`; `sampler_backoff` on the lead returns `{ok:true}` so a stale in-memory backoff cannot fail a single-model request with zero live attempt. An unreachable wait loop after that return is deleted. Same-link retry after a real attempt still records in-flight failures; the unused storm-bail helper is gone.

`buildAutoChainLive` skips `refreshAll`/`refreshAcpModels` when `targetModel` is a specific non-`auto` id (handleChat's pinned path). Auto/unknown still refreshes. `GET /v1/models` runs ollama, chatjimmy, and ACP probes concurrently; a dead ollama is cached 30s.
