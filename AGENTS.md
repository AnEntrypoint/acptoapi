...
### Post-extraction models filter (over-enumeration guard)

Borrowing another host's `/models` endpoint (as `codestral` does from `api.mistral.ai`) means the enumerated catalog answers "what exists on this key's account", not "what THIS brand's chat URL will actually accept" -- the two can diverge. Witnessed live (2026-07-22, real `CODESTRAL_API_KEY`): the codestral catalog returns 60 ids (every mistral-family model reachable by that key), but `codestral.mistral.ai/v1/chat/completions` 403s `tier_not_allowed` on any non-codestral-branded id (`mistral-medium-2505`, `devstral-latest`, ...) and 400s `invalid_model` on the two embeddings-only ids (`codestral-embed*`) -- only `codestral-2508`/`codestral-latest` are genuinely chat-servable. Left unfiltered, the other 58 occupied real `buildAutoChain` slots guaranteed to fail; each failure's `reason=auth`/`reason=error` cascaded `sampler_backoff` across the rest of the mistral/codestral pool (the sibling-prefix exemption in chain-machine.js does not save a SAME-prefix run of 58 bad ids). `MODELS_FILTER` (brand-catalog.js, keyed the same way as `MODELS_URL_OVERRIDE`) applies a post-`_extractIds` predicate before caching: `codestral: (id) => /^codestral/i.test(id) && !/embed/i.test(id)`. Absent/`false` for a brand means every enumerated id is trusted as-is (the default, correct for a brand enumerating its OWN native endpoint -- `zai`'s catalog was spot-checked live and returns only its own `glm-*` ids, not a borrowed-host over-enumeration case).
...
Two independent boot paths start the readiness prober, gated by two DIFFERENT env vars — do not use one where the other's caller applies:

- **Standalone HTTP daemon** (`lib/server.js`'s `server.listen` callback, the `acptoapi` CLI/`createServer()` process): starts `readiness.start()` (idempotent single interval, unref'd) plus one warm-up `runOnce()` ~8s after boot (after the brand-catalog refresh populates candidates), gated by `ACPTOAPI_DISABLE_BOOT_PROBE`/`ACPTOAPI_DISABLE_PROBE`/**`ACPTOAPI_DISABLE_READINESS=1`** (opt out; default on).
- **In-process SDK** (`index.js`'s `ensureReadinessStarted`, what `sdk.chat()`/`sdk.chatChain()` trigger on first real call — the only path a consumer that never runs the standalone server, e.g. freddie, ever reaches): gated by **`ACPTOAPI_READINESS_DISABLE=1`** (`startOnceOnFirstRealCall('ACPTOAPI_READINESS_DISABLE', ...)`, `index.js:156`) — note the different variable name and word order from the standalone-daemon var above. Setting `ACPTOAPI_DISABLE_READINESS` has no effect on this path; setting `ACPTOAPI_READINESS_DISABLE` has no effect on the standalone daemon's own boot sequence. A consumer wanting to suppress readiness on both paths sets both vars.

`GET /v1/readiness` returns `{ candidates: [{model, lastProbeTs, ok, latencyMs, rank, fresh, nextProbeInMs}] }`. Env: `ACPTOAPI_READINESS_INTERVAL_MS` (default 120000), `ACPTOAPI_READINESS_FRESH_MS` (90000), `ACPTOAPI_READINESS_TOPK` (5), `ACPTOAPI_READINESS_MAX_PER_PROVIDER` (2), `ACPTOAPI_READINESS_PROBE_TIMEOUT_MS` (8000), `ACPTOAPI_READINESS_SPACING_MS` (200).

### xai-oauth probe carve-out (2026-08-20)

`probeOne`'s generic path is `sdk.chat({model, ...})`, which resolves auth via `keyring.getKey(r.env)` (lib/sdk.js's `buildParams`)  - correct for every keyed brand, but `xai-oauth`'s `BUILTIN_KEYS` entry is `null` (its credential is an on-disk OAuth token store, lib/xai-oauth.js, not an env var  - see the xAI Grok OAuth section above). `resolveModel('xai-oauth/...')` therefore resolves `env: null`, `buildParams` never calls `keyring.getKey`, and every probe sent `Authorization: Bearer undefined`, which x.ai correctly 401s with `"Incorrect API key provided"`  - a structural false negative from the first probe pass onward, indistinguishable in the logs from a genuine auth failure. `probeOne` now special-cases `model.startsWith('xai-oauth/')` to call `xaiOauth.chatCompletion(...)` (the same credential-aware, reactive-refresh-on-401 helper `executeXaiOauthModel` uses for real traffic) instead of `sdk.chat`, passing `{ maxRetries: 0, quiet: true }`  - `maxRetries: 0` skips the multi-attempt 429/5xx backoff loop (a probe's own ~2min interval is already the retry cadence; without this a probe that hits its outer `Promise.race` timeout could leave a detached backoff loop still retrying in the background) while still performing the single unconditional 401-triggered refresh-and-retry. `quiet: true` suppresses the per-attempt `console.log` lines real traffic gets (a probe firing every ~2min would otherwise be as noisy as live request traffic for routine retries). See the last-link bypass note under Error Classification above for how this false-negative signal, left unpatched, was able to permanently gate the only configured model off of every future request.
...
`.app-main a` descendant rule outweighs `.btn-*` single-class selectors; buttons must stay excluded via `:not()` on the anchor rule or they render invisible. Detail in rs-learn (recall "acptoapi-app-shell-css-specificity").
...
`FALLBACK_REASONS` (chain-machine.js:4) = `['error', 'timeout', 'rate_limit', 'empty', 'content_policy', 'sampler_backoff', 'matrix_block', 'auth', 'fetch_failed', 'credit_dead']`.

`classifyError(err)` (chain-machine.js:8) maps a thrown error to a reason:
- `status === 402` or `err.code === 'CREDIT_DEAD'` or message matches `/402|creditserror|no payment method|insufficient credits|payment.?required/i` -> `credit_dead` (2026-08-31: distinguishes "no payment method on this account" from a bare `error`, so a provider with zero credits gets the same hard-exclusion treatment as an invalid key rather than occupying a chain slot on every request)
...
**`credit_dead` propagation (2026-08-31).** `PROVIDER_LEVEL_HEALTH_REASONS` (chain-machine.js:42) includes `credit_dead`, so `markProviderFailed` also calls `require('./brand-catalog').markCreditDead(provider)` on that reason — this persists to the SAME `~/.acptoapi/brand-catalog-cache.json` entry `isAuthDead` already used for 401/403, under a new `creditDead`/`creditDeadAt` field, checked via `brandCatalog.isCreditDead(name)`/`isDead(name)` (auth-dead OR credit-dead). `buildAutoChain` (auto-chain.js:416, :448) excludes a `isDead()` brand from the chain pool entirely — same hard-exclusion semantics as the existing auth-dead path, not just a rank demotion. This closes a gap `brandCatalog.probeBrand`'s catalog-only probe structurally cannot see for a brand like `opencode-zen`, whose `/models` endpoint is billing-blind (returns a clean 200 model list regardless of payment status) while every real chat call 402s `CreditsError` — `probeBrand` was also hardened to detect a `CreditsError`/`no payment method` body embedded in an HTTP 200 response (`reason: 'http_402'`), for the rarer case where the catalog endpoint itself surfaces billing state.

`lib/named-chains.js`'s and `lib/auto-chain.js`'s `FALLBACK_ON` constants (see "Terminal vs. trigger-next-link" below) both now include `credit_dead` so it advances the chain rather than surfacing to the caller when another link remains.
...
A fourth `preCheck` reason, `model_unhealthy`, is NOT in `FALLBACK_REASONS` (it never needs to be  - it is produced before a link is invoked, same as `sampler_backoff`/`matrix_block`, not returned by `classifyError`): `peek(link.model).failStreak >= MIN_FAILSTREAK_TO_SKIP_MODEL` (`ACPTOAPI_MODEL_SKIP_FAILSTREAK`, default 5) AND `availability.score(link.model) < 0` (lib/availability.js, chain-machine.js:96). Per-**model** (not per-prefix like the sampler breaker), driven by `lib/availability.js`'s persisted failStreak/score, so this gate — unlike `sampler_backoff`, which is pure in-memory and always clean after a restart — survives process restarts via `~/.acptoapi/availability-cache.json`.

**Last-link bypass (2026-08-20).** All three `preCheck` pre-flight gates (`sampler_backoff`, `model_unhealthy`, `matrix_block`) are skipped unconditionally when the link being checked is the last one in the chain (`isLastLink`, passed by `runChat`/`runStream` as `snap.context.index >= links.length - 1`  - the identical condition the xstate machine's own `hasMore` guard uses to decide there is no next link). Root cause this closed: a single-provider chain (e.g. `xai-oauth/grok-4.6` pinned with no other configured provider) that racked up 5+ consecutive failures from ANY cause  - including a structurally broken health-probe path that was never even sending valid credentials, see readiness.js below  - got `model_unhealthy`-blocked on every future request with **zero live attempt**, and because `availability-cache.json` persists across restarts, a server restart did not clear it: the chain was permanently DOA until someone manually edited or deleted the cache file, regardless of whether the provider had actually recovered (or had never actually been down). Skipping a pre-flight gate only ever makes sense when a better candidate is waiting behind it to save the caller a slow doomed attempt; with zero remaining candidates, skipping guarantees total failure while attempting can only be equal-or-better (either it now succeeds, self-healing the cached health score via the normal `recordSuccess` path, or it fails exactly as a skip would have, but with a real error instead of a stale guess). This is a request-shape correctness fix, not a "keep trying more providers" fallback: a single-provider chain still has exactly one provider and no alternate model is ever substituted.

`peekNext()` (lib/chain.js:95, the `/v1/...` diagnostic "next-up" inspection helper documented under Inspection helpers) deliberately does NOT apply this bypass  - it calls `preCheck(l, opts)` with no third argument, so it still honestly reports what WOULD block a link for inspection purposes, independent of whether that link happens to be last.
...
- Built-in named chains (`lib/named-chains.js:15`) pin `fallbackOn = ['error', 'rate_limit', 'timeout', 'empty', 'credit_dead']` per link  - `auth`/`content_policy`/`fetch_failed` are terminal for those chains.
- `buildAutoChain` links pin the full 9-item set including `credit_dead` (see "Canonical reason set" above).
...
## Chain fluidity: wall-clock budget, cross-request storm detection, real streaming (2026-08-31)

Four compounding gaps made a real `/v1/chat/completions` request occasionally hang for 5+ minutes with zero bytes reaching the client, even though every individual timeout/retry budget in the chain was itself reasonable:

1. **`availability.js` latency penalty was under-weighted.** `score()`'s `latencyPenalty` capped at 10 while `successBonus` caps at `SUCCESS_STREAK_CAP` (30) -- a 50s-average model with a long success streak (30-10=20) outranked a 2.2s-average model with fewer successes (3-2=1). Fixed: `LATENCY_PENALTY_CAP_MS` (`ACPTOAPI_AVAILABILITY_LATENCY_PENALTY_CAP_MS`, default `SUCCESS_STREAK_CAP * 1000`) now matches the same ceiling as the success bonus, so an extreme outlier latency can fully cancel out even a maxed success streak.
2. **`model_unhealthy` had no bounded wait.** `waitForLeadLinkPrecheck` (chain-machine.js) previously waited the FULL `SAME_LINK_RETRY_BUDGET_MS` on a `model_unhealthy` block identically to `sampler_backoff` -- but unlike `sampler_backoff` (a real, precise `nextRetryAt` deadline via `sampler.peekStatus`), `model_unhealthy` is a persisted, no-ETA health signal (`availability.js`'s `effectiveFailStreak`/`rank`) that can stay negative indefinitely. Fixed: bounded to its own much shorter `MODEL_UNHEALTHY_WAIT_BUDGET_MS` (`ACPTOAPI_MODEL_UNHEALTHY_WAIT_BUDGET_MS`, default 10s).
3. **No cross-request visibility into an in-flight same-link retry storm.** `availability.recordFailure` only fires once a same-link retry loop fully exhausts (not per individual retry attempt), so a SECOND concurrent request had zero visibility into "this exact model is mid-storm right now" until the first request's entire budget ran out -- witnessed two concurrent requests each independently retrying the same erroring lead model for 100+ seconds. Fixed: an in-process, in-flight failure tracker (`inFlightFailures` Map, chain-machine.js) updated on EVERY same-link retry attempt (not just final exhaustion); `isInFlightStorming(model)` (threshold `ACPTOAPI_INFLIGHT_STORM_THRESHOLD`, default 3, within `ACPTOAPI_INFLIGHT_STORM_WINDOW_MS`, default 15s) is checked by `waitForLeadLinkPrecheck` as a fast bail-out for a later request racing the same model.
4. **No overall chain wall-clock ceiling.** Each per-link budget was individually reasonable but nothing bounded their SUM across a 9-12 link auto chain -- witnessed a request walk ~64 attempts across 5 links over 5+ minutes. Fixed: `CHAIN_WALL_CLOCK_BUDGET_MS` (`ACPTOAPI_CHAIN_WALL_CLOCK_BUDGET_MS`, default 90s, or per-call `opts.chainBudgetMs`) checked at the top of both `runStream`/`runChat`'s main loop and threaded into every same-link-retry condition and into `waitForLeadLinkPrecheck` -- once the deadline passes, the chain throws `CHAIN_BUDGET_EXCEEDED` immediately rather than continuing to wait on any remaining link or sleeping past it.

### In-flight storm tracker reopened the last-link fake-response gap (2026-09-01)

`waitForLeadLinkPrecheck`'s in-flight-storm fast path (item 3 above) fires whenever ANY earlier request tripped `isInFlightStorming(model)` for that exact model id, unconditionally at the top of the function, before ever calling `preCheck`. For a single-model direct request (or the final fallback link), the one link is BOTH the lead link and the last link -- but the storm check has no last-link exemption, so it returns `{ok:false, reason:'model_unhealthy'}` with **zero live attempt**, indistinguishable from every other exhausted-chain case: the caller gets a fabricated `200 {"content":"All upstream providers are currently unavailable..."}` even though the model was never actually tried this request. Witnessed live: `openrouter/anthropic/claude-sonnet-5:batch`, storm-flagged by an earlier session's testing, returned this fake response with the process barely a few seconds old and no upstream call in the log. Fixed: `waitForLeadLinkPrecheck` takes an `isLastLink` param (`snap.context.index >= snap.context.links.length - 1`, computed at both call sites in `runStream`/`runChat`) and short-circuits straight to `preCheck(link, opts)` (skipping the storm fast-path and the whole wait loop) whenever the lead link has no candidate behind it -- same reasoning as the wall-clock/model_unhealthy fixes above: with zero remaining candidates, a real attempt is only ever equal-or-better than a synthetic block.

### openrouter `:batch`-suffixed models are never chat-servable (2026-09-01)

openrouter's `/v1/models` catalog enumerates `:batch`-suffixed ids (e.g. `anthropic/claude-sonnet-5:batch`) alongside normal ones, but its synchronous `/chat/completions` endpoint 404s ALL of them unconditionally: `"This model is only available through the Batch API. Use the /api/beta/batches endpoint instead."` -- witnessed across every `:batch` id the readiness prober touched. Same over-enumeration shape as the codestral case documented above (a catalog answering "what exists" rather than "what this endpoint serves"): left unfiltered, `:batch` ids occupied real `buildAutoChain` slots guaranteed to 404 on first (and every) real attempt, burning the full same-link retry budget (several 15s waits) before falling through. Fixed via the same `MODELS_FILTER` mechanism (`lib/brand-catalog.js`): `openrouter: (id) => !/:batch$/i.test(id)`. Witnessed live: openrouter's catalog count dropped 420 -> 355 (65 `:batch` ids removed) and the live 12-link auto-chain no longer contains any `:batch` id, filling the freed ranking slots with genuinely servable free-tier models instead.

**Two env-var naming collisions, fixed:**
- `ACPTOAPI_CHAIN_LINK_TIMEOUT_MS` (chain-machine.js's own `DEFAULT_LINK_TIMEOUT_MS` fallback) is only used when a caller passes no explicit `opts.timeout` -- but `server.js`'s `handleChat` (the real `/v1/chat/completions` handler) always computes and passes its OWN `linkTimeout` from `ACPTOAPI_LINK_TIMEOUT_MS`, which overrides the chain-machine default entirely. A deployment that set only `ACPTOAPI_CHAIN_LINK_TIMEOUT_MS` (a name that reads as "the" chain link timeout) had ZERO effect on real chat-completions traffic. `handleChat`'s `linkTimeout` now falls back to `ACPTOAPI_CHAIN_LINK_TIMEOUT_MS` too, before the hardcoded `120000` default.
- `laguna-s-2.1-free`/`nemotron-3-ultra-free`-class free models can have a genuine (not broken) response time in the tens of seconds -- a too-short `ACPTOAPI_LINK_TIMEOUT_MS` cuts a real, working, slow model off before it ever finishes, forcing endless same-link retries that each ALSO hit the same wall. Tune this value to the slowest genuinely-reliable free model in your pool's real response time, not an arbitrary "fast" number.

**Real incremental streaming wired into `/v1/chat/completions` (previously fake).** `handleChat`'s streaming branch ALWAYS ran the full non-streamed `runChat` path internally and synthesized ONE fake SSE chunk from the complete result at the end -- a client received zero bytes for the ENTIRE real response time of a slow model, indistinguishable from no streaming at all (confirmed by the code's own prior comment: "Synthesize an SSE stream from the (non-streamed) chain result"). The real machinery already existed and was already used elsewhere (`lib/sdk.js`'s `stream()` -> `translate()` -> `lib/providers/openai.js`'s `streamOpenAI()`, genuine chunk-by-chunk SSE parsing against the real upstream) but was never wired into this HTTP route. Now: streaming requests run `chain-machine.js`'s `runStream` with a `streamFn` wrapping `sdk.stream()`, forwarding real `text-delta`/`tool-call` events as they arrive. Non-streaming requests (and their response-cache wrap) are completely unchanged.

**Streaming header parity.** `res.writeHead` is deferred (`ensureHeaders()`, idempotent) until the served model is known (set inside the streamFn callback, only fires once a link is chosen) or exhaustion is caught -- previously headers fired immediately with neither known yet, so a streamed response never carried `X-Acptoapi-Served-Model`/`X-Acptoapi-Chain-Attempts` (success) or `X-Acptoapi-Chain-Exhausted` (exhaustion), unlike the non-streaming path which always had them.
